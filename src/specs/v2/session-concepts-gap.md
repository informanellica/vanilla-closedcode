...
Compared with `packages/closedcode/src/session/message-v2.ts` and `packages/closedcode/src/session/processor.ts`, `packages/closedcode/src/v2` currently captures the rough event stream for prompts, assistant steps, text, reasoning, tools, retries, and compaction, but it does not yet capture several persisted-message and processor concepts.
