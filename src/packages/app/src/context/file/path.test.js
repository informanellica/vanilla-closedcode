...
import { createPathHelpers, stripQueryAndHash, unquoteGitPath, encodeFilePath } from "./path.js";
...
describe("encodeFilePath", () => {
  ...
  describe("macOS paths", () => {
    test("should handle macOS absolute path", () => {
      const macPath = "/Users/alex/Projects/closedcode/README.md";
      const result = encodeFilePath(macPath);
      const fileUrl = `file://${result}`;
      expect(() => new URL(fileUrl)).not.toThrow();
      expect(result).toBe("/Users/alex/Projects/closedcode/README.md");
    });
    test("should handle macOS path with spaces", () => {
      const macPath = "/Users/alex/My Documents/file.txt";
      const result = encodeFilePath(macPath);
      const fileUrl = `file://${result}`;
      expect(() => new URL(fileUrl)).not.toThrow();
  ...
  describe("Windows paths", () => {
    test("should handle Windows absolute path with backslashes", () => {
      const windowsPath = "D:\\dev\\projects\\closedcode\\README.bs.md";
      ...
      expect(result).toBe("/D:/dev/projects/closedcode/README.bs.md");
    });
    test("should handle mixed separator path (Windows + Unix)", () => {
      ...
      const mixedPath = "D:\\dev\\projects\\closedcode/README.bs.md";
    ...
    test("should NOT create invalid URL like the bug report", () => {
      ...
      // The bug was creating: file://D%3A%5Cdev%5Cprojects%5Cclosedcode/README.bs.md
      expect(result).not.toContain("%5C"); // Should not have encoded backslashes
      expect(result).not.toBe("D%3A%5Cdev%5Cprojects%5Cclosedcode/README.bs.md");
