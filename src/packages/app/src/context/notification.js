...
export const {
  ...
  init: () => {
    ...
    const [store, setStore, _, ready] = persisted(Persist.global("notification", ["notification.v1"]), createStore({
      list: []
    }));
    const [index, setIndex] = createStore(buildNotificationIndex(store.list));
    const meta = {
      pruned: false,
      disposed: false
    };
    ...
    createEffect(() => {
      ...
      batch(() => {
        setStore("list", list);
        setIndex(reconcile(buildNotificationIndex(list), {
          merge: false
        }));
      });
    });
    ...
    const lookup = async (directory, sessionID) => {
      ...
      return globalSDK.client.session.get({
        directory,
        sessionID
    ...
    const handleSessionError = (directory, event, time) => {
      ...
      void lookup(directory, sessionID).then(session => {
        ...
        const description = session?.title ?? (typeof error === "string" ? error : language.t("notification.session.error.fallbackDescription"));
    ...
    return {
      ...
      session: {
        ...
        unseenHasError(session) {
          return index.session.unseenHasError[session] ?? false;
        },
        markViewed(session) {
          const unseen = index.session.unseen[session] ?? empty;
          if (!unseen.length) return;
          const projects = [...new Set(unseen.flatMap(notification => notification.directory ? [notification.directory] : []))];
          batch(() => {
            setStore("list", n => n.session === session && !n.viewed, "viewed", true);
            updateUnseen("session", session, []);
            projects.forEach(directory => {
              const next = (index.project.unseen[directory] ?? empty).filter(notification => notification.session !== session);
      ...
      project: {
        ...
        unseenCount(directory) {
          return index.project.unseenCount[directory] ?? 0;
        },
        unseenHasError(directory) {
          return index.project.unseenHasError[directory] ?? false;
        },
        markViewed(directory) {
          const unseen = index.project.unseen[directory] ?? empty;
          if (!unseen.length) return;
          const sessions = [...new Set(unseen.flatMap(notification => notification.session ? [notification.session] : []))];
          batch(() => {
            setStore("list", n => n.directory === directory && !n.viewed, "viewed", true);
            updateUnseen("project", directory, []);
            sessions.forEach(session => {
              const next = (index.session.unseen[session] ?? empty).filter(notification => notification.directory !== directory);
