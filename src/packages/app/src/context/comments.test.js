...
describe("comments session indexing", () => {
  test("keeps file list behavior and aggregate chronological order", () => {
    createRoot(dispose => {
      ...
      expect(comments.all().map(item => item.time)).toEqual(comments.all().map(item => item.time).slice().sort((a, b) => a - b));
      dispose();
    });
  });
  test("remove updates file and aggregate indexes consistently", () => {
    createRoot(dispose => {
      ...
      expect(comments.all().filter(item => item.id === "shared").map(item => item.file)).toEqual(["b.ts"]);
      expect(comments.focus()).toBeNull();
      expect(comments.active()).toEqual({
        file: "a.ts",
        id: "shared"
      });
      dispose();
