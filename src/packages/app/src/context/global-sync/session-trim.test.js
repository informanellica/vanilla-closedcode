...
describe("trimSessions", () => {
  ...
  test("keeps children when root is kept, permission exists, or child is recent", () => {
    ...
    const list = [session({
      ...
      id: "z-root",
      created: now - 30_000_000
    }), session({
      id: "child-kept-by-root",
      parentID: "root-1",
      created: now - 20_000_000
    }), session({
      id: "child-kept-by-permission",
      parentID: "z-root",
      created: now - 20_000_000
    }), session({
      id: "child-kept-by-recency",
      parentID: "z-root",
      created: now - 500
    }), session({
      id: "child-trimmed",
    ...
    expect(result.map(x => x.id)).toEqual(["child-kept-by-permission", "child-kept-by-recency", "child-kept-by-root", "root-1", "root-2"]);
  });
});