...
const userMessage = (id, sessionID) => ({
  id,
  sessionID,
  role: "user",
  time: {
...
const textPart = (id, sessionID, messageID) => ({
  id,
  sessionID,
  messageID,
  type: "text",
...
const permissionRequest = (id, sessionID, title = id) => ({
  id,
  sessionID,
  permission: title,
  patterns: ["*"],
...
const questionRequest = (id, sessionID, title = id) => ({
  id,
  sessionID,
  questions: [{
    question: title,
...
describe("applyDirectoryEvent", () => {
  ...
  test("cleans session caches when archived", () => {
    ...
    const [store, setStore] = createStore(baseState({
      ...
      part: {
        [message.id]: [textPart("prt_1", "ses_1", message.id)]
      },
      session_diff: {
        ses_1: []
      },
  ...
  test("cleans session caches when deleted and decrements only root totals", () => {
    ...
    for (const item of cases) {
      ...
      const [store, setStore] = createStore(baseState({
        ...
        part: {
          [message.id]: [textPart("prt_1", item.info.id, message.id)]
        },
        session_diff: {
          [item.info.id]: []
        },
  ...
  test("cleans caches for trimmed sessions on session.created", () => {
    const dropped = rootSession({
      id: "ses_b"
    });
    const kept = rootSession({
      id: "ses_a"
    });
    ...
    const [store, setStore] = createStore(baseState({
      ...
      part: {
        [message.id]: [textPart("prt_1", dropped.id, message.id)]
      },
      session_diff: {
        [dropped.id]: []
      },
  ...
  test("cleanupDroppedSessionCaches clears part-only orphan state", () => {
    const [store, setStore] = createStore(baseState({
      ...
      part: {
        msg_1: [textPart("prt_1", "ses_drop", "msg_1")]
      }
    }));
  ...
  test("upserts and removes messages while clearing orphaned parts", () => {
    ...
    const [store, setStore] = createStore(baseState({
      ...
      part: {
        msg_2: [textPart("prt_1", sessionID, "msg_2")]
      }
    }));
    applyDirectoryEvent({
      event: {
        ...
        properties: {
          info: userMessage("msg_2", sessionID)
        }
      },
      store,
      setStore,
  ...
  test("upserts and prunes message parts", () => {
    ...
    const [store, setStore] = createStore(baseState({
      part: {
        [messageID]: [textPart("prt_1", sessionID, messageID), textPart("prt_3", sessionID, messageID)]
      }
    }));
    applyDirectoryEvent({
      event: {
        ...
        properties: {
          part: textPart("prt_2", sessionID, messageID)
        }
      },
      store,
      setStore,
  ...
  test("tracks permission and question request lifecycles", () => {
    ...
    const [store, setStore] = createStore(baseState({
      permission: {
        [sessionID]: [permissionRequest("perm_1", sessionID), permissionRequest("perm_3", sessionID)]
      },
      question: {
