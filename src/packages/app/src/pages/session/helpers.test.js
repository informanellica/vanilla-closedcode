...
describe("createOpenSessionFileTab", () => {
  test("activates the opened file tab", () => {
    ...
    openTab("src/a.ts");
    expect(calls).toEqual(["normalize:src/a.ts", "open:file://src/a.ts", "path:file://src/a.ts", "load:src/a.ts", "review", "active:file://src/a.ts"]);
  });
});
...
describe("shouldFocusTerminalOnKeyDown", () => {
  test("skips pure modifier keys", () => {
    expect(shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", {
      key: "Meta",
      metaKey: true
    }))).toBe(false);
    expect(shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", {
