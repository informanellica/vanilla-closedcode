...
export function resolveThemeVariant(variant, isDark) {
  ...
  const amber = generateScale(shift(colors.warning, isDark ? {
    h: -16,
    l: -0.058,
    c: 1.14
  } : {
  ...
  const blue = generateScale(shift(colors.interactive, {
    h: -12,
    l: 0.128,
    c: 1.12
  }), isDark);
  const diffAdd = generateScale(colors.diffAdd ?? shift(colors.success, {
    c: isDark ? 0.7 : 0.55,
    l: isDark ? -0.18 : 0.14
  }), isDark);
  const diffDelete = generateScale(colors.diffDelete ?? shift(colors.error, {
    c: isDark ? 0.82 : 0.7,
    l: isDark ? -0.08 : 0.08
  }), isDark);
  ...
  const content = (seed, scale) => {
    ...
    const value = isDark ? base.l > 0.84 ? shift(seed, {
      c: 1.18
    }) : scale[10] : scale[10];
    return shift(value, {
      l: isDark ? 0.034 : -0.024,
      c: isDark ? 1.3 : 1.18
    });
  };
  const modified = () => {
  ...
  const alphaTone = (color, alpha) => overlay ? withAlpha(color, alpha) : blend(color, background, alpha);
  const borderTone = (light, dark) => alphaTone(ink, isDark ? Math.min(1, dark + 0.024 + (colors.compact ? 0.08 : 0)) : Math.min(1, light + 0.024));
  const diffHiddenSurface = surface(isDark ? shift(colors.interactive, {
    c: 0.55,
    l: 0
  }) : shift(colors.interactive, {
    c: 0.45,
    l: 0.08
  }), isDark ? {
    base: 0.14,
  ...
  tokens["text-weak"] = colors.compact ? shift(body, {
    l: isDark ? -0.11 : 0.11,
    c: 0.9
  }) : neutral[8];
  tokens["text-weaker"] = colors.compact ? shift(body, {
    l: isDark ? -0.2 : 0.21,
    c: isDark ? 0.78 : 0.72
  }) : neutral[7];
  tokens["text-strong"] = colors.compact ? isDark ? blend("#ffffff", body, 0.9) : shift(body, {
    l: -0.07,
    c: 1.04
  }) : neutral[11];
