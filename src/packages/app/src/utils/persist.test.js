...
class MemoryStorage {
  ...
  getItem(key) {
    ...
    if (key.startsWith("closedcode.throw")) throw new Error("storage get failed");
    return this.values.get(key) ?? null;
  }
  setItem(key, value) {
    this.calls.set += 1;
    this.events.push(`set:${key}`);
    if (key.startsWith("closedcode.quota")) throw new DOMException("quota", "QuotaExceededError");
    if (key.startsWith("closedcode.throw")) throw new Error("storage set failed");
    this.values.set(key, value);
  }
  removeItem(key) {
    this.calls.remove += 1;
    this.events.push(`remove:${key}`);
    if (key.startsWith("closedcode.throw")) throw new Error("storage remove failed");
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("closedcode.quota.scope");
    storageApi.setItem("value", '{"value":1}');
    expect(storage.getItem("closedcode.quota.scope:value")).toBeNull();
    expect(storageApi.getItem("value")).toBeNull();
  });
  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("closedcode.throw.scope");
    ...
    const healthy = persistTesting.localStorageWithPrefix("closedcode.safe.scope");
    healthy.setItem("value", '{"value":3}');
    expect(storage.getItem("closedcode.safe.scope:value")).toBe('{"value":3}');
  });
  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("closedcode.throw.scope2");
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("closedcode.workspace.");
