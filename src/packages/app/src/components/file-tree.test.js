...
beforeAll(async () => {
  ...
  mock.module("ui/collapsible", () => ({
    Collapsible: {
      Trigger: props => props.children,
      Content: props => props.children
    }
  }));
  mock.module("ui/file-icon", () => ({
    FileIcon: () => null
  }));
  mock.module("ui/icon", () => ({
    Icon: () => null
  ...
  const mod = await import("./file-tree.js");
...
describe("file tree fetch discipline", () => {
  ...
  test("allowed auto-expand picks only collapsed dirs", () => {
    ...
    const filter = {
      dirs: new Set(["src", "src/components"])
    };
