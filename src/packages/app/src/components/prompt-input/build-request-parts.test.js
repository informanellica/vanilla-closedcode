...
describe("buildRequestParts", () => {
  test("builds typed request and optimistic parts without cast path", () => {
    ...
    expect(result.requestParts.some(part => part.type === "file" && part.url.startsWith("file:///repo/src/foo.ts"))).toBe(true);
    expect(result.requestParts.some(part => part.type === "text" && part.synthetic)).toBe(true);
    expect(result.requestParts.some(part => part.type === "text" && part.synthetic && part.metadata?.closedcodeComment && part.metadata.closedcodeComment.comment === "check this")).toBe(true);
  ...
  test("deduplicates context files when prompt already includes same path", () => {
    ...
    const fooFiles = result.requestParts.filter(part => part.type === "file" && part.url.startsWith("file:///repo/src/foo.ts"));
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/alex/Projects/closedcode"
    });
    const filePart = result.requestParts.find(part => part.type === "file");
    expect(filePart).toBeDefined();
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow();
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/alex/Projects/closedcode/README.md");
    }
  });
