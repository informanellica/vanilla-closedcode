...
const STORAGE_KEYS = {
  THEME_ID: "closedcode-theme-id",
  COLOR_SCHEME: "closedcode-color-scheme",
  THEME_CSS_LIGHT: "closedcode-theme-css-light",
  THEME_CSS_DARK: "closedcode-theme-css-dark"
};
...
function getFiles() {
  ...
  files = import.meta.glob("./themes/*.json");
  return files;
}
function themeIDs() {
  if (ids) return ids;
  ids = Object.keys(getFiles()).map(path => path.slice("./themes/".length, -".json".length)).sort();
  return ids;
}
...
function normalize(id) {
  ...
  if (id === "opencode") return "closedcode";
  return id;
}
function read(key) {
...
function applyThemeCss(theme, themeId, mode) {
...
function cacheThemeVariants(theme, themeId) {
...
export const {
  ...
  name: "Theme",
  init: props => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? read("opencode-theme-id") ?? props.defaultTheme) ?? "oc-2";
    const colorScheme = read(STORAGE_KEYS.COLOR_SCHEME) ?? read("opencode-color-scheme") ?? "system";
    ...
    const applyTheme = (theme, themeId, mode) => {
      applyThemeCss(theme, themeId, mode);
      props.onThemeApplied?.(theme, mode);
    };
    const ids = () => {
      const extra = Object.keys(store.themes).filter(id => !knownThemes().has(id)).sort();
      const all = themeIDs();
    ...
    onMount(() => {
      ...
      const savedScheme = read(STORAGE_KEYS.COLOR_SCHEME) ?? "system";
    ...
    return {
      ...
      mode: () => store.mode,
      ids,
      name: id => store.themes[id]?.name ?? names[id] ?? id,
      loadThemes,
      themes: () => store.themes,
      setTheme,
      setColorScheme,
      registerTheme: theme => setStore("themes", theme.id, theme),
      previewTheme: id => {
        const next = normalize(id);
        if (!next) return;
        if (next !== "oc-2" && !knownThemes().has(next) && !store.themes[next]) return;
        ...
        void load(next).then(theme => {
          ...
          const mode = store.previewScheme ? store.previewScheme === "system" ? getSystemMode() : store.previewScheme : store.mode;
