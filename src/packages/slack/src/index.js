...
console.log("🚀 Starting closedcode server...");
const opencode = await createOpencode({
  port: 0
});
console.log("✅ ClosedCode server ready");
...
app.message(async ({
  ...
  if (!session) {
    console.log("🆕 Creating new closedcode session...");
    const {
      client,
      server
    } = opencode;
    ...
    console.log("✅ Created closedcode session:", createResult.data.id);
    session = {
      client,
      server,
  ...
  console.log("📝 Sending to closedcode:", message.text);
  ...
  console.log("📤 ClosedCode response:", JSON.stringify(result, null, 2));
  if (result.error) {
  ...
  // Build response text
  const responseText = response.info?.content || response.parts?.filter(p => p.type === "text").map(p => p.text).join("\n") || "I received your message but didn't have a response.";
