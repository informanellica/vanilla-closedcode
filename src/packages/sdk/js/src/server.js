/**
 * @file SDK process launchers. Spawns the `closedcode` binary either as a headless
 * HTTP server ({@link createOpencodeServer}) or as an interactive TUI
...
export async function createOpencodeServer(options) {
  ...
  const proc = launch(`closedcode`, args, {
    env: {
      ...process.env,
      CLOSEDCODE_CONFIG_CONTENT: JSON.stringify(options.config ?? {})
    }
  });
  let clear = () => {};
  const url = await new Promise((resolve, reject) => {
    ...
    proc.stdout?.on("data", chunk => {
      ...
      for (const line of lines) {
        if (line.startsWith("closedcode server listening")) {
