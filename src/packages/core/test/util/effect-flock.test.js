...
const worker = path.join(__dirname, "../fixture/effect-flock-worker.js");
function run(msg) {
...
async function waitForFile(file, timeout = 3_000) {
...
describe("util.effect-flock", () => {
  ...
  it.live("acquire and release via scoped Effect", Effect.gen(function* () {
    ...
    yield* Effect.promise(() => fs.rm(tmp, {
      recursive: true,
      force: true
    }));
  }));
  it.live("withLock data-first", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
    let hit = false;
    yield* flock.withLock(Effect.sync(() => {
      hit = true;
    }), "eflock:df", dir);
  ...
  it.live("withLock pipeable", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
    let hit = false;
  ...
  it.live("writes owner metadata", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
    ...
    yield* Effect.scoped(Effect.gen(function* () {
      ...
      const json = yield* Effect.promise(() => readJson(file));
  ...
  it.live("breaks stale lock dirs", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
  ...
  it.live("recovers from stale breaker", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
  ...
  it.live("detects compromise when lock dir removed", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
    const key = "eflock:compromised";
    const lockDir = lock(dir, key);
    const result = yield* flock.withLock(Effect.promise(() => fs.rm(lockDir, {
      recursive: true,
      force: true
    })), key, dir).pipe(Effect.exit);
  ...
  it.live("detects token mismatch", Effect.gen(function* () {
    const flock = yield* EffectFlock.Service;
    const tmp = yield* Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "eflock-test-")));
    const dir = path.join(tmp, "locks");
    ...
    const result = yield* flock.withLock(Effect.promise(async () => {
      const json = await readJson(meta);
      json.token = "tampered";
  ...
  it.live("fails on unwritable lock roots", Effect.gen(function* () {
    ...
    yield* Effect.promise(() => fs.chmod(dir, 0o700).then(() => fs.rm(tmp, {
      recursive: true,
      force: true
    })));
  }));
  it.live("enforces mutual exclusion under process contention", () => Effect.promise(async () => {
  ...
  it.live("recovers after a crashed lock owner", () => Effect.promise(async () => {
