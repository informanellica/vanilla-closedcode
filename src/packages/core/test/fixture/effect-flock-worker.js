...
async function job() {
  ...
  if (msg.active) await fs.writeFile(msg.active, String(process.pid), {
    flag: "wx"
  });
  try {
    ...
    if (msg.active) await fs.rm(msg.active, {
      force: true
    });
  }
}
await Effect.runPromise(Effect.gen(function* () {
  ...
  yield* flock.withLock(Effect.promise(() => job()), msg.key, msg.dir);
}).pipe(Effect.provide(testLayer))).catch(err => {
