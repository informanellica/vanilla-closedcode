...
- Author package entrypoint is `plugin/tui`.
...
{
  "theme": "smoke-theme",
  "plugin": ["@acme/closedcode-plugin@1.2.3", ["./plugins/demo.js", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `plugin/tui`.
- `plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- Path specs like `./plugin` can resolve to `./plugin/index.js` (or `index.mjs` / `index.cjs`) when `package.json` is missing.
...
- The value is a semver range checked against the running closedcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- `engines.closedcode` is the canonical field; `engines.opencode` is read as a legacy fallback.
- If neither field is present, no check is performed (backward compatible).
- File plugins are never checked; only npm package plugins are validated.

- Install flow is shared by CLI and TUI in `src/plugin/install.js`.
- Shared helpers are `installPlugin`, `readPluginManifest`, and `patchPluginConfig`.
- `closedcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `closedcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.closedcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.closedcode`.
- Legacy `.opencode` fallback remains covered by regression tests.
...
When `plugin` entries exist in a writable `.closedcode` dir or `CLOSEDCODE_CONFIG_DIR`, closedcode installs `plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `plugin/tui`.
...
- Local plugins persist installed themes under the local `.closedcode/themes` area near the plugin config source.
...
- `--pure` / `CLOSEDCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.closedcode/plugins/tui-smoke.js`
- Local vim plugin: `.closedcode/plugins/tui-vim.js`
- Local smoke config: `.closedcode/tui.json`
- Local smoke theme: `.closedcode/plugins/smoke-theme.json`
