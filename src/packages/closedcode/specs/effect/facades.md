...
Concrete inventory of the remaining `makeRuntime(...)`-backed facades in `packages/closedcode`.

Current status on this branch:

- `src/` has 5 `makeRuntime(...)` call sites total.
- 2 are intentionally excluded from this checklist: `src/bus/index.js` and `src/effect/cross-spawn-spawner.js`.
- 1 is tracked primarily by the instance-context migration rather than facade removal: `src/project/instance.js`.
- That leaves 2 live runtime-backed service facades still worth tracking here: `src/npm/index.js` and `src/cli/cmd/tui/config/tui.js`.
...
- `src/cli/cmd/tui/config/tui.js` still exports `makeRuntime(...)` plus async facade helpers for `get()` and `waitForDependencies()`.
- `src/npm/index.js` still exports `makeRuntime(...)` plus async facade helpers for `install()`, `add()`, `outdated()`, and `which()`.
- `src/project/instance.js` still uses a dedicated runtime for project boot, but that file is really part of the broader legacy instance-context transition tracked in `instance-context.md`.
...
1. `src/pty/index.js`
2. `src/skill/index.js`
3. `src/project/vcs.js`
4. `src/tool/registry.js`
5. `src/auth/index.js`

Caller-heavy batch, all merged:

1. `src/config/config.js`
2. `src/provider/provider.js`
3. `src/file/index.js`
4. `src/lsp/index.js`
5. `src/mcp/index.js`
...
1. remove the `Npm` runtime-backed facade from `src/npm/index.js`
2. remove the `TuiConfig` runtime-backed facade from `src/cli/cmd/tui/config/tui.js`
3. keep `src/project/instance.js` in the separate instance-context migration, not this checklist

## Checklist

- [ ] `src/npm/index.js` (`Npm`) - still exports runtime-backed async facade helpers on top of `Npm.Service`
- [ ] `src/cli/cmd/tui/config/tui.js` (`TuiConfig`) - still exports runtime-backed async facade helpers on top of `TuiConfig.Service`
- [x] `src/session/session.js` / `src/session/prompt.js` / `src/session/revert.js` / `src/session/summary.js` - service-local facades removed
- [x] `src/agent/agent.js` (`Agent`) - service-local facades removed
- [x] `src/permission/index.js` (`Permission`) - service-local facades removed
- [x] `src/worktree/index.js` (`Worktree`) - service-local facades removed
- [x] `src/plugin/index.js` (`Plugin`) - service-local facades removed
- [x] `src/snapshot/index.js` (`Snapshot`) - service-local facades removed
- [x] `src/file/index.js` (`File`) - facades removed and merged
- [x] `src/lsp/index.js` (`LSP`) - facades removed and merged
- [x] `src/mcp/index.js` (`MCP`) - facades removed and merged
- [x] `src/config/config.js` (`Config`) - facades removed and merged
- [x] `src/provider/provider.js` (`Provider`) - facades removed and merged
- [x] `src/pty/index.js` (`Pty`) - facades removed and merged
- [x] `src/skill/index.js` (`Skill`) - facades removed and merged
- [x] `src/project/vcs.js` (`Vcs`) - facades removed and merged
- [x] `src/tool/registry.js` (`ToolRegistry`) - facades removed and merged
- [x] `src/auth/index.js` (`Auth`) - facades removed and merged

## Excluded `makeRuntime(...)` sites

- `src/bus/index.js` - core bus plumbing, not a normal facade-removal target.
- `src/effect/cross-spawn-spawner.js` - runtime helper for `ChildProcessSpawner`, not a service namespace facade.
