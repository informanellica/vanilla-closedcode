...
Practical reference for converting server route handlers in `packages/closedcode` to a single `AppRuntime.runPromise(Effect.gen(...))` body.
...
- [x] `server/routes/instance/question.js` — handlers yield `Question.Service`
- [x] `server/routes/instance/provider.js` — handlers yield `Provider.Service`, `ProviderAuth.Service`, and `Config.Service`
- [x] `server/routes/instance/permission.js` — handlers yield `Permission.Service`
- [x] `server/routes/instance/mcp.js` — handlers mostly yield `MCP.Service`
- [x] `server/routes/instance/pty.js` — handlers yield `Pty.Service`

Files still worth tracking here:

- [ ] `server/routes/instance/session.js` — still the heaviest mixed file; many handlers are composed, but the file still mixes patterns and has direct `Bus.publish(...)` / `Session.list(...)` usage
- [ ] `server/routes/instance/index.js` — mostly converted, but still has direct `Instance.dispose()` / `Instance.*` reads for `/instance/dispose` and `/path`
- [ ] `server/routes/instance/file.js` — most handlers yield services, but `/find` still passes `Instance.directory` directly into ripgrep and `/find/symbol` is still stubbed
- [ ] `server/routes/instance/experimental.js` — mixed state; many handlers are composed, but some still rely on `runRequest(...)` or direct `Instance.project` reads
- [ ] `server/routes/instance/middleware.js` — still enters the instance via `Instance.provide(...)`
- [ ] `server/routes/global.js` — still uses `Instance.disposeAll()` and remains partly outside the fully-composed style
