This is a plugin to simulate a remote environment locally. Add this to `.closedcode/closedcode.jsonc` (legacy: `.opencode/opencode.jsonc`):

```json
  "plugin": ["../packages/closedcode/src/control-plane/dev/debug-workspace-plugin.js"],
```

In a separate terminal, run a separate closedcode server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/closedcode/script/run-workspace-server
```

With the plugin install, you can now run closedcode and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
