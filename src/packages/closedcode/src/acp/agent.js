...
async function getContextLimit(sdk, providerID, modelID, directory) {
  const providers = await sdk.config.providers({
    directory
  }).then(x => x.data?.providers ?? []).catch(error => {
    log.error("failed to get providers for context limit", {
      error
    });
    return [];
  });
...
async function sendUsageUpdate(connection, sdk, sessionID, directory) {
  ...
  await connection.sessionUpdate({
    ...
    log.error("failed to send usage update", {
      error
    });
  });
}
...
export class Agent {
  ...
  startEventSubscription() {
    ...
    this.runEventSubscription().catch(error => {
      ...
      log.error("event subscription failed", {
        error
      });
    });
  }
      ...
      case "permission.asked":
        {
          ...
          const next = prev.then(async () => {
            ...
            log.error("failed to handle permission", {
              error,
              permissionID: permission.id
            });
          }).finally(() => {
                      ...
                      await this.connection.sessionUpdate({
                        ...
                        update: {
                          ...
                          entries: parsedTodos.success.map(todo => {
                            const status = todo.status === "cancelled" ? "completed" : todo.status;
                            return {
                              priority: "medium",
                              status,
      ...
      case "message.part.delta":
        {
          ...
          if (part.type === "text" && props.field === "text" && part.ignored !== true) {
            await this.connection.sessionUpdate({
              ...
              log.error("failed to send text delta to ACP", {
                error
              });
            });
            return;
          ...
          if (part.type === "reasoning" && props.field === "text") {
            await this.connection.sessionUpdate({
              ...
              log.error("failed to send reasoning delta to ACP", {
                error
              });
            });
          }
  ...
  async initialize(params) {
    ...
    const authMethod = {
      description: "Run `closedcode auth login` in the terminal",
      name: "Login with closedcode",
      id: "closedcode-login"
    };
  ...
  async listSessions(params) {
    try {
      ...
      const entries = page.map(session => ({
        sessionId: session.id,
  ...
  async processMessage(message) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        if (part.text) {
          const audience = part.synthetic ? ["assistant"] : part.ignored ? ["user"] : undefined;
          await this.connection.sessionUpdate({
            sessionId,
            update: {
        ...
        // ClosedCode stores files internally as { type: "file", url, filename, mime }.
        ...
        if (url.startsWith("file://")) {
          ...
          await this.connection.sessionUpdate({
            ...
            log.error("failed to send resource_link to ACP", {
              error: err
            });
          });
          ...
          if (effectiveMime.startsWith("image/")) {
            ...
            await this.connection.sessionUpdate({
              ...
              log.error("failed to send image to ACP", {
                error: err
              });
            });
          } else {
            ...
            await this.connection.sessionUpdate({
              ...
              log.error("failed to send resource to ACP", {
                error: err
              });
            });
          }
  ...
  async loadAvailableModes(directory) {
    ...
    return agents.filter(agent => agent.mode !== "subagent" && !agent.hidden).map(agent => ({
      id: agent.name,
      name: agent.name,
  ...
  async loadSessionMode(params) {
    ...
    const commands = await this.config.sdk.command.list({
      directory
    }, {
      throwOnError: true
    }).then(resp => resp.data);
    ...
    if (!names.has("compact")) availableCommands.push({
      name: "compact",
    ...
    for (const server of params.mcpServers) {
      if ("type" in server) {
        mcpServers[server.name] = {
          url: server.url,
          headers: server.headers.reduce((acc, {
            name,
            value
          }) => {
            acc[name] = value;
        ...
        mcpServers[server.name] = {
          ...
          environment: server.env.reduce((acc, {
            name,
            value
          }) => {
            acc[name] = value;
    ...
    await Promise.all(Object.entries(mcpServers).map(async ([key, mcp]) => {
      await this.sdk.mcp.add({
        ...
        log.error("failed to add mcp server", {
          name: key,
          error
        });
      });
    ...
    if (params.configId === "model") {
      ...
      if (!availableModes.some(mode => mode.id === params.value)) {
        throw RequestError.invalidParams(JSON.stringify({
          error: `Mode not found: ${params.value}`
        }));
      }
      this.sessionManager.setMode(session.id, params.value);
    } else {
      throw RequestError.invalidParams(JSON.stringify({
        error: `Unknown config option: ${params.configId}`
      }));
    }
...
async function defaultModel(config, cwd) {
  ...
  const specified = await sdk.config.get({
    ...
    log.error("failed to load user config for default model", {
      error
    });
    return undefined;
  });
  const providers = await sdk.config.providers({
    ...
    log.error("failed to list providers for default model", {
      error
    });
    return [];
  });
...
function parseModelSelection(modelId, providers) {
  ...
  // Try to extract variant from end of modelID (e.g., "qwen/qwen3-coder/high" -> model: "qwen/qwen3-coder", variant: "high")
