...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const state = yield* InstanceState.make(Effect.fn("ProviderAuth.state")(function* () {
    const plugins = yield* plugin.list();
    return {
      hooks: Record.fromEntries(Arr.filterMap(plugins, x => x.auth?.provider !== undefined ? Result.succeed([ProviderID.make(x.auth.provider), x.auth]) : Result.failVoid)),
      pending: new Map()
    };
  }));
    ...
    if (method.prompts && input.inputs) {
      for (const prompt of method.prompts) {
        if (prompt.type === "text" && prompt.validate && input.inputs[prompt.key] !== undefined) {
          ...
          if (error) return yield* Effect.fail(new ValidationFailed({
            field: prompt.key,
            message: error
          }));
        }
  ...
  const callback = Effect.fn("ProviderAuth.callback")(function* (input) {
    ...
    if (!match) return yield* Effect.fail(new OauthMissing({
      providerID: input.providerID
    }));
    if (match.method === "code" && !input.code) {
      return yield* Effect.fail(new OauthCodeMissing({
        providerID: input.providerID
      }));
    }
    const result = yield* Effect.promise(() => match.method === "code" ? match.callback(input.code) : match.callback());
