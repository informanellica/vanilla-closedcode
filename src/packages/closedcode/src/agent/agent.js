...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const state = yield* InstanceState.make(Effect.fn("Agent.state")(function* (ctx) {
    ...
    const agents = {
      ...
      plan: {
        ...
        permission: Permission.merge(defaults, Permission.fromConfig({
          ...
          edit: {
            "*": "deny",
            [path.join(".closedcode", "plans", "*.md")]: "allow",
    ...
    for (const name in agents) {
      ...
      agents[name].permission = Permission.merge(agents[name].permission, Permission.fromConfig({
        external_directory: {
          [Truncate.GLOB]: "allow"
        }
      }));
  ...
  return Service.of({
    ...
    generate: Effect.fn("Agent.generate")(function* (input) {
      ...
      const params = {
        ...
        temperature: 0.3,
        messages: [...(isOpenaiOauth ? [] : system.map(item => ({
          role: "system",
          content: item
        }))), {
          role: "user",
