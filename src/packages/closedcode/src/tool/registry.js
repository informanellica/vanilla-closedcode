...
import z from "zod";
import { ZodOverride } from "@/util/effect-zod.js";
...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const state = yield* InstanceState.make(Effect.fn("ToolRegistry.state")(function* (ctx) {
    ...
    function fromPlugin(id, def) {
      ...
      const parameters = Schema.declare(u => zodParams.safeParse(u).success).annotate({
    ...
    const matches = dirs.flatMap(dir => Glob.scanSync("{tool,tools}/*.{js,ts}", {
      cwd: dir,
      absolute: true,
      dot: true,
      symlink: true
    ...
    yield* config.get();
    const questionEnabled = ["app", "cli", "desktop"].includes(Flag.CLOSEDCODE_CLIENT) || Flag.CLOSEDCODE_ENABLE_QUESTION_TOOL;
  ...
  const tools = Effect.fn("ToolRegistry.tools")(function* (input) {
    ...
    return yield* Effect.forEach(filtered, Effect.fnUntraced(function* (tool) {
      ...
      yield* plugin.trigger("tool.definition", {
        toolID: tool.id
      }, output);
      return {
        id: tool.id,
        description: [output.description, tool.id === TaskTool.id ? yield* describeTask(input.agent) : undefined, tool.id === SkillTool.id ? yield* describeSkill(input.agent) : undefined].filter(Boolean).join("\n"),
