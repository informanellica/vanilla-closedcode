  ...
  const run = Effect.fn("ApplyPatchTool.execute")(function* (params, ctx) {
    ...
    for (const hunk of hunks) {
      ...
      switch (hunk.type) {
        ...
        case "update":
          {
            ...
            if (!stats || stats.type === "Directory") {
              return yield* Effect.fail(new Error(`apply_patch verification failed: Failed to read file to update: ${filePath}`));
            }
        ...
        case "delete":
          {
            const source = yield* Bom.readFile(afs, filePath).pipe(Effect.catch(error => Effect.fail(new Error(`apply_patch verification failed: ${error instanceof Error ? error.message : String(error)}`))));
