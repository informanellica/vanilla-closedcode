  ...
  return {
    ...
    execute: (params, ctx) => Effect.gen(function* () {
      ...
      const files = yield* rg.files({
        ...
        glob: [params.pattern],
        signal: ctx.abort
      }).pipe(Stream.mapEffect(file => Effect.gen(function* () {
        const full = path.resolve(search, file);
        const info = yield* fs.stat(full).pipe(Effect.catch(() => Effect.succeed(undefined)));
        const mtime = info?.mtime.pipe(Option.map(date => date.getTime()), Option.getOrElse(() => 0)) ?? 0;
        return {
          path: full,
          mtime
        };
      })), Stream.take(limit + 1), Stream.runCollect, Effect.map(chunk => [...chunk]));
