...
export const GrepDefinition = Effect.gen(function* () {
  ...
  return {
    ...
    execute: (params, ctx) => Effect.gen(function* () {
      ...
      const search = AppFileSystem.resolve(path.isAbsolute(params.path ?? ins.directory) ? params.path ?? ins.directory : path.join(ins.directory, params.path ?? "."));
      ...
      const times = new Map((yield* Effect.forEach([...new Set(rows.map(row => row.path))], Effect.fnUntraced(function* (file) {
        const info = yield* fs.stat(file).pipe(Effect.catch(() => Effect.succeed(undefined)));
        if (!info || info.type === "Directory") return undefined;
        return [file, info.mtime.pipe(Option.map(time => time.getTime()), Option.getOrElse(() => 0)) ?? 0];
      }), {
        concurrency: 16
