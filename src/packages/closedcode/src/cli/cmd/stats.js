...
export const StatsCommand = effectCmd({
  ...
  builder: yargs => yargs.option("days", {
...
const aggregateSessionStats = Effect.fn("Cli.stats.aggregate")(function* (days, projectFilter, currentProject) {
  ...
  const results = yield* Effect.forEach(filteredSessions, session => Effect.gen(function* () {
    ...
    let sessionToolUsage = {};
    let sessionModelUsage = {};
    for (const message of messages) {
      if (message.info.role === "assistant") {
        ...
        if (message.info.tokens) {
          ...
          sessionModelUsage[modelKey].tokens.output += (message.info.tokens.output || 0) + (message.info.tokens.reasoning || 0);
