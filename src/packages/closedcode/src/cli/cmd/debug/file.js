...
const FileSearchCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.debug.file.search")(function* (args) {
    const results = yield* File.Service.use(svc => svc.search({
      query: args.query
    }));
...
export const FileCommand = cmd({
  ...
  builder: yargs => yargs.command(FileReadCommand).command(FileStatusCommand).command(FileListCommand).command(FileSearchCommand).command(FileTreeCommand).demandCommand(),
  async handler() {}
});