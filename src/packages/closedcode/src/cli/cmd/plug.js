  ...
  return async ctx => {
    ...
    if (!manifest.ok) {
      ...
      if (manifest.code === "manifest_no_targets") {
        ...
        dep.log.info('Expected one of: exports["./tui"], exports["./server"], package.json main for server, or package.json["oc-themes"] for tui themes.');
        return false;
      }
      inspect.stop("Manifest read failed", 1);
      return false;
    }
    inspect.stop(`Detected ${manifest.targets.map(item => item.kind).join(" + ")} target${manifest.targets.length === 1 ? "" : "s"}`);
...
export const PluginCommand = effectCmd({
  ...
  builder: yargs => yargs.positional("module", {
    type: "string",
  ...
  handler: Effect.fn("Cli.plug")(function* (args) {
    ...
    if (!ctx) return;
    const ok = yield* Effect.promise(() => run({
      vcs: ctx.project.vcs,
      worktree: ctx.worktree,
      directory: ctx.directory
    }));
