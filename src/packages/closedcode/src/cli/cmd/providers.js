...
async function handlePluginAuth(plugin, provider, methodName) {
  ...
  if (methodName) {
    ...
    if (match === -1) {
      prompts.log.error(`Unknown method "${methodName}" for ${provider}. Available: ${plugin.auth.methods.map(x => x.label).join(", ")}`);
      process.exit(1);
    }
    index = match;
  ...
  if (method.type === "api") {
    ...
    const metadata = Object.keys(inputs).length ? {
      metadata: inputs
    } : {};
    if (!method.authorize) {
      await put(provider, {
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const options = [...pipe(providers, values(), sortBy(x => priority[x.id] ?? 99, x => x.name ?? x.id), map(x => ({
        label: x.name,
        value: x.id,
        hint: undefined
      ...
      if (provider === "other") {
        ...
        prompts.log.warn(`This only stores a credential for ${provider} - you will need to configure it in closedcode.json, check the docs for examples.`);
      }
      const key = await prompts.password({
        message: "Enter your API key",
        validate: x => x && x.length > 0 ? undefined : "Required"
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (_args) {
    ...
    yield* Effect.promise(async () => {
      UI.empty();
      const credentials = Object.entries(await Effect.runPromise(authSvc.all()));
