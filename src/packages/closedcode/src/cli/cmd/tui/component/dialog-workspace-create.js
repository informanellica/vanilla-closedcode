...
export async function restoreWorkspaceSession(input) {
  const result = await input.sdk.client.experimental.workspace.sessionRestore({
...
export function DialogWorkspaceCreate(props) {
  ...
  const [adapters, setAdapters] = createSignal();
  onMount(() => {
    ...
    void (async () => {
      ...
      const res = await sdk.fetch(url).then(x => x.json()).catch(() => undefined);
      if (!res) {
        toast.show({
  ...
  return _$createComponent(DialogSelect, {
    get title() {
      return creating() ? "Creating Workspace" : "New Workspace";
    },
    skipFilter: true,
    get options() {
      return options();
