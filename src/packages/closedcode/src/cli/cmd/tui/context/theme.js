...
import closedcode from "./theme/closedcode.json" with { type: "json" };
...
export function resolveTheme(theme, mode) {
  ...
  const resolved = Object.fromEntries(Object.entries(theme.theme).filter(([key]) => key !== "selectedListItemText" && key !== "backgroundMenu" && key !== "thinkingOpacity").map(([key, value]) => {
...
export const {
  ...
  init: props => {
    ...
    setStore(produce(draft => {
      ...
      draft.mode = mode;
      draft.lock = lock;
      const raw = config.theme ?? kv.get("theme", "closedcode");
      const active = typeof raw === "string" ? raw : "closedcode";
      // Legacy compat: users with "opencode" in config/KV get the renamed theme.
      draft.active = active === "opencode" ? "closedcode" : active;
      draft.ready = false;
    }));
    createEffect(() => {
    ...
    function init() {
      void Promise.allSettled([resolveSystemTheme(store.mode), getCustomThemes().then(custom => {
        customThemes = custom;
        syncThemes();
      }).catch(() => {
        setStore("active", "closedcode");
      })]).finally(() => {
        setStore("ready", true);
      });
    }
    onMount(init);
    function resolveSystemTheme(mode = store.mode) {
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.closedcode, store.mode);
    });
    createEffect(() => {
...
async function getCustomThemes() {
  const directories = [Global.Path.config, ...(await Array.fromAsync(Filesystem.up({
...
function generateSyntax(theme) {
  return SyntaxStyle.fromTheme(getSyntaxRules(theme));
}
function generateSubtleSyntax(theme) {
