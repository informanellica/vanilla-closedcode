...
function activeAssistant(messages) {
  const index = messages.findLastIndex(message => message.type === "assistant" && !message.time.completed);
  if (index < 0) return;
  const assistant = messages[index];
  return assistant?.type === "assistant" ? assistant : undefined;
}
function activeCompaction(messages) {
  const index = messages.findLastIndex(message => message.type === "compaction");
  if (index < 0) return;
  const compaction = messages[index];
  return compaction?.type === "compaction" ? compaction : undefined;
}
function activeShell(messages, callID) {
  const index = messages.findLastIndex(message => message.type === "shell" && message.callID === callID);
  if (index < 0) return;
  const shell = messages[index];
  return shell?.type === "shell" ? shell : undefined;
}
function latestTool(assistant, callID) {
  return assistant?.content.findLast(item => item.type === "tool" && (callID === undefined || item.id === callID));
}
function latestText(assistant) {
  return assistant?.content.findLast(item => item.type === "text");
}
function latestReasoning(assistant, reasoningID) {
  return assistant?.content.findLast(item => item.type === "reasoning" && item.id === reasoningID);
}
export const {
  use: useSyncV2,
  ...
  init: () => {
    ...
    const sdk = useSDK();
    function update(sessionID, fn) {
      setStore("messages", produce(draft => {
        fn(draft[sessionID] ??= []);
      }));
    }
      ...
      switch (event.type) {
        ...
        case "session.next.step.ended":
          update(event.properties.sessionID, draft => {
            ...
            if (event.properties.snapshot) currentAssistant.snapshot = {
