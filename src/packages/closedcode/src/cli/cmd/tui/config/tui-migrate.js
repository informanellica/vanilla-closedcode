...
export async function migrateTuiConfig(input) {
  ...
  for (const file of opencode) {
    ...
    const wrote = await Filesystem.write(target, JSON.stringify(payload, null, 2)).then(() => true).catch(error => {
...
async function backupAndStripLegacy(file, source) {
  ...
  const backed = hasBackup ? true : await Filesystem.write(backup, source).then(() => true).catch(error => {
  ...
  return Filesystem.write(file, text).then(() => {
...
async function opencodeFiles(input) {
  ...
  if (Flag.CLOSEDCODE_CONFIG) files.push(Flag.CLOSEDCODE_CONFIG);
  ...
  return existing.filter(file => !!file);
}