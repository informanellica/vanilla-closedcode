...
export async function read() {
  const os = platform();
  if (os === "darwin") {
    const tmpfile = path.join(tmpdir(), "closedcode-clipboard.png");
    try {
      await Process.run(["osascript", "-e", 'set imageData to the clipboard as "PNGf"', "-e", `set fileRef to open for access POSIX file "${tmpfile}" with write permission`, "-e", "set eof fileRef to 0", "-e", "write imageData to fileRef", "-e", "close access fileRef"], {
        nothrow: true
      });
  ...
  if (os === "linux") {
    ...
    if (wayland.stdout.byteLength > 0) {
      return {
        data: Buffer.from(wayland.stdout).toString("base64"),
        mime: "image/png"
      };
    }
    ...
    if (x11.stdout.byteLength > 0) {
      return {
        data: Buffer.from(x11.stdout).toString("base64"),
        mime: "image/png"
      };
    }
  }
...
const getCopyMethod = lazy(async () => {
  ...
  if (os === "win32") {
    ...
    return async text => {
      ...
      const proc = Process.spawn(["powershell.exe", "-NonInteractive", "-NoProfile", "-Command", "[Console]::InputEncoding = [System.Text.Encoding]::UTF8; Set-Clipboard -Value ([Console]::In.ReadToEnd())"], {
        stdin: "pipe",
        stdout: "ignore",
        stderr: "ignore"
      });
