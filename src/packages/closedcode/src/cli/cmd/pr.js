...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const checkout = yield* Effect.promise(() => Process.run(["gh", "pr", "checkout", `${prNumber}`, "--branch", localBranchName, "--force"], {
      nothrow: true
    }));
    if (checkout.code !== 0) {
      return yield* fail(`Failed to checkout PR #${prNumber}. Make sure you have gh CLI installed and authenticated.`);
    }
    const prInfoResult = yield* Effect.promise(() => Process.text(["gh", "pr", "view", `${prNumber}`, "--json", "headRepository,headRepositoryOwner,isCrossRepository,headRefName,body"], {
      nothrow: true
    }));
    let sessionId;
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.isCrossRepository && prInfo.headRepository && prInfo.headRepositoryOwner) {
        ...
        const remotes = (yield* git.run(["remote"], {
          cwd: worktree
        })).text().trim();
        if (!remotes.split("\n").includes(remoteName)) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found closedcode session: ${sessionUrl}`);
          UI.println(`Importing session...`);
          const importResult = yield* Effect.promise(() => Process.text(["closedcode", "import", sessionUrl], {
            nothrow: true
          }));
    ...
    UI.println();
    UI.println("Starting closedcode...");
    UI.println();
    const closedcodeArgs = sessionId ? ["-s", sessionId] : [];
    const code = yield* Effect.promise(() => Process.spawn(["closedcode", ...closedcodeArgs], {
      stdin: "inherit",
      stdout: "inherit",
      stderr: "inherit",
      cwd: process.cwd()
    ...
    if (code !== 0) return yield* Effect.die(new Error(`closedcode exited with code ${code}`));
  })
});