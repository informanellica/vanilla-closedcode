...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: closedcode mcp add");
      return;
    }
    for (const [name, serverConfig] of servers) {
      ...
      prompts.log.info(`${statusIcon} ${name} ${UI.Style.TEXT_DIM}${statusText}${hint}\n    ${UI.Style.TEXT_DIM}${typeHint}`);
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in closedcode.json:");
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
    ...
    if (authStatus === "authenticated") {
      const confirm = yield* Effect.promise(() => prompts.confirm({
    ...
    yield* MCP.Service.use(mcp => mcp.authenticate(serverName)).pipe(Effect.tap(status => Effect.sync(() => {
    ...
    if (!serverName) {
      const selected = yield* Effect.promise(() => prompts.select({
        ...
        options: serverNames.map(name => {
          ...
          let hint = "";
          if (hasTokens && hasClient) hint = "tokens + client";else if (hasTokens) hint = "tokens";else if (hasClient) hint = "client registration";
          return {
            label: name,
            value: name,
            hint
...
export const McpAddCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.add")(function* () {
    ...
    yield* Effect.promise(async () => {
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([resolveConfigPath(ctx.worktree), resolveConfigPath(Global.Path.config, true)]);
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., closedcode x @modelcontextprotocol/server-filesystem",
          validate: x => x && x.length > 0 ? undefined : "Required"
        });
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        ...
        if (response.status === 401) {
          ...
          const authProvider = new McpOAuthProvider(serverName, serverConfig.url, {
