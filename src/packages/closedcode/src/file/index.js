...
const sortHiddenLast = (items, prefer) => {
  ...
  for (const item of items) {
    if (hidden(item)) hiddenItems.push(item);else visible.push(item);
  }
...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const state = yield* InstanceState.make(Effect.fn("File.state")(() => Effect.succeed({
    cache: {
      files: [],
      dirs: []
    }
  ...
  const status = Effect.fn("File.status")(function* () {
    ...
    const diffOutput = yield* gitText(["-c", "core.fsmonitor=false", "-c", "core.quotepath=false", "diff", "--numstat", "HEAD"]);
    const changed = [];
    if (diffOutput.trim()) {
    ...
    const untrackedOutput = yield* gitText(["-c", "core.fsmonitor=false", "-c", "core.quotepath=false", "ls-files", "--others", "--exclude-standard"]);
    if (untrackedOutput.trim()) {
      for (const file of untrackedOutput.trim().split("\n")) {
        const content = yield* appFs.readFileString(path.join(ctx.directory, file)).pipe(Effect.catch(() => Effect.succeed(undefined)));
    ...
    const deletedOutput = yield* gitText(["-c", "core.fsmonitor=false", "-c", "core.quotepath=false", "diff", "--name-only", "--diff-filter=D", "HEAD"]);
  ...
  const read = Effect.fn("File.read")(function* (file) {
    ...
    if (isBinaryByExtension(file) && !knownText) return {
      type: "binary",
      content: ""
    };
    const exists = yield* appFs.existsSafe(full);
    ...
    if (encode && !isImage(mimeType)) return {
      type: "binary",
      content: "",
      mimeType
    };
    ...
    const content = yield* appFs.readFileString(full).pipe(Effect.map(s => s.trim()), Effect.catch(() => Effect.succeed("")));
  ...
  const search = Effect.fn("File.search")(function* (input) {
    ...
    const sorted = fuzzysort.go(query, items, {
      limit: searchLimit
...
export const defaultLayer = layer.pipe(Layer.provide(Ripgrep.defaultLayer), Layer.provide(AppFileSystem.defaultLayer), Layer.provide(Git.defaultLayer));
export * as File from "./index.js";