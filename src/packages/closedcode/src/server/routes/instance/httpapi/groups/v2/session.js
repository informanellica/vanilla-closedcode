...
export const SessionGroup = HttpApiGroup.make("v2.session").add(HttpApiEndpoint.get("sessions", "/api/session", {
  query: Schema.Union([Schema.Struct({
    ...
    roots: Schema.Literals(["true", "false"]).pipe(Schema.decodeTo(Schema.Boolean, {
...
}))).add(HttpApiEndpoint.post("prompt", "/api/session/:sessionID/prompt", {
  params: {
    sessionID: SessionID
  },
...
}))).add(HttpApiEndpoint.post("compact", "/api/session/:sessionID/compact", {
  params: {
    sessionID: SessionID
  },
...
}))).add(HttpApiEndpoint.post("wait", "/api/session/:sessionID/wait", {
  params: {
    sessionID: SessionID
  },
  success: HttpApiSchema.NoContent
...
}))).add(HttpApiEndpoint.get("context", "/api/session/:sessionID/context", {
  params: {
    sessionID: SessionID
  },
