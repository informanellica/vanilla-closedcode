...
const PROMPT_PLAN = assetText("session/prompt/plan.txt");
const BUILD_SWITCH = assetText("session/prompt/build-switch.txt");
const MAX_STEPS = assetText("session/prompt/max-steps.txt");
...
import { decodeDataUrl } from "#util/data-url.js";
...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const resolvePromptParts = Effect.fn("SessionPrompt.resolvePromptParts")(function* (template) {
    ...
    const seen = new Set();
    yield* Effect.forEach(files, Effect.fnUntraced(function* (match) {
      const name = match[1];
      if (seen.has(name)) return;
      seen.add(name);
      const filepath = name.startsWith("~/") ? path.join(os.homedir(), name.slice(2)) : path.resolve(ctx.worktree, name);
  ...
  const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input) {
    ...
    const msgs = onlySubtasks ? [{
      role: "user",
      content: subtasks.map(p => p.prompt).join("\n")
    ...
    }).pipe(Stream.filter(e => e.type === "text-delta"), Stream.map(e => e.text), Stream.mkString, Effect.orDie);
    ...
    yield* sessions.setTitle({
      sessionID: input.session.id,
      title: t
    }).pipe(Effect.catchCause(cause => elog.error("failed to generate title", {
  ...
  const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input) {
    ...
    if (!Flag.CLOSEDCODE_EXPERIMENTAL_PLAN_MODE) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
  ...
  const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input) {
    ...
    const context = (args, options) => ({
      ...
      metadata: val => input.processor.updateToolCall(options.toolCallId, match => {
    ...
    for (const [key, item] of Object.entries(yield* mcp.tools())) {
      ...
      item.execute = (args, opts) => run.promise(Effect.gen(function* () {
        ...
        for (const contentItem of result.content) {
          if (contentItem.type === "text") textParts.push(contentItem.text);else if (contentItem.type === "image") {
            attachments.push({
              type: "file",
  ...
  const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input) {
    ...
    const result = yield* taskTool.execute(taskArgs, {
      ...
      messages: msgs,
      metadata: val => Effect.gen(function* () {
  ...
  const shellImpl = Effect.fn("SessionPrompt.shellImpl")(function* (input, ready) {
    return yield* Effect.uninterruptibleMask(restore => Effect.gen(function* () {
      ...
      const exit = yield* restore(Effect.gen(function* () {
        const shellEnv = yield* plugin.trigger("shell.env", {
          cwd,
          sessionID: input.sessionID,
          callID: part.callID
        }, {
        ...
        yield* Stream.runForEach(Stream.decodeText(handle.all), chunk => Effect.gen(function* () {
          output += chunk;
          if (part.state.status === "running") {
            part.state.metadata = {
              output,
      ...
      }).pipe(Effect.scoped, Effect.orDie)).pipe(Effect.exit);
  ...
  const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input) {
    ...
    const full = !input.variant && ag.variant && same ? yield* provider.getModel(model.providerID, model.modelID).pipe(Effect.catchDefect(() => Effect.void)) : undefined;
          ...
          case "data:":
            if (part.mime === "text/plain") {
              return [{
                ...
                type: "text",
                synthetic: true,
                text: `Called the Read tool with the following input: ${JSON.stringify({
          ...
          case "file:":
            {
              ...
              if (mime === "text/plain") {
                ...
                if (range.start != null) {
                  ...
                  if (start === end) {
                    ...
                    for (const symbol of symbols) {
                      let r;
                      if ("range" in symbol) r = symbol.range;else if ("location" in symbol) r = symbol.location.range;
                ...
                const exit = yield* provider.getModel(info.model.providerID, info.model.modelID).pipe(Effect.flatMap(mdl => execRead(args, {
                  model: mdl
                })), Effect.exit);
                if (Exit.isSuccess(exit)) {
                  ...
                  if (result.attachments?.length) {
                    pieces.push(...result.attachments.map(a => ({
                      ...a,
                      synthetic: true,
    ...
    const nextPrompt = parts.reduce((result, part) => {
      if (part.type === "text") {
        if (part.synthetic) result.synthetic.push(part.text);else result.text.push(part.text);
      }
      if (part.type === "file") {
        result.files.push(new FileAttachment({
          uri: part.url,
          mime: part.mime,
          name: part.filename,
          source: part.source ? new Source({
      ...
      if (part.type === "agent") {
        result.agents.push(new AgentAttachment({
          name: part.name,
          source: part.source ? new Source({
  ...
  const runLoop = Effect.fn("SessionPrompt.run")(function* (sessionID) {
    ...
    while (true) {
      ...
      if (lastAssistant?.finish && !["tool-calls"].includes(lastAssistant.finish) && !hasToolCalls && lastUser.id < lastAssistant.id) {
        yield* slog.info("exiting loop");
        break;
      }
      step++;
      if (step === 1) yield* title({
        session,
      ...
      if (lastFinished && lastFinished.summary !== true && (yield* compaction.isOverflow({
      ...
      const outcome = yield* Effect.gen(function* () {
        ...
        if (step === 1) yield* summary.summarize({
          sessionID,
        ...
        if (step > 1 && lastFinished) {
          for (const m of msgs) {
            ...
            for (const p of m.parts) {
              ...
              p.text = ["<system-reminder>", "The user sent the following message:", p.text, "", "Please address this message and continue with your tasks.", "</system-reminder>"].join("\n");
            }
          }
        }
        ...
        const [skills, env, instructions, modelMsgs] = yield* Effect.all([sys.skills(agent), sys.environment(model), instruction.system().pipe(Effect.orDie), MessageV2.toModelMessagesEffect(msgs, model)]);
        const system = [...env, ...instructions, ...(skills ? [skills] : [])];
        const format = lastUser.format ?? {
          type: "text"
        };
  ...
  const command = Effect.fn("SessionPrompt.command")(function* (input) {
    ...
    if (shellMatches.length > 0) {
      const cfg = yield* config.get();
      const sh = Shell.preferred(cfg.shell);
      const results = yield* Effect.promise(() => Promise.all(shellMatches.map(async ([, cmd]) => (await Process.text([cmd], {
        shell: sh,
        nothrow: true
      })).text)));
      let index = 0;
    ...
    yield* plugin.trigger("command.execute.before", {
      command: input.command,
      sessionID: input.sessionID,
...
export const defaultLayer = Layer.suspend(() => layer.pipe(Layer.provide(SessionRunState.defaultLayer), Layer.provide(SessionStatus.defaultLayer), Layer.provide(SessionCompaction.defaultLayer), Layer.provide(SessionProcessor.defaultLayer), Layer.provide(Command.defaultLayer), Layer.provide(Permission.defaultLayer), Layer.provide(MCP.defaultLayer), Layer.provide(LSP.defaultLayer), Layer.provide(ToolRegistry.defaultLayer), Layer.provide(Truncate.defaultLayer), Layer.provide(Provider.defaultLayer), Layer.provide(Config.defaultLayer), Layer.provide(Instruction.defaultLayer), Layer.provide(AppFileSystem.defaultLayer), Layer.provide(Plugin.defaultLayer), Layer.provide(Session.defaultLayer), Layer.provide(SessionRevert.defaultLayer), Layer.provide(SessionSummary.defaultLayer), Layer.provide(Layer.mergeAll(Agent.defaultLayer, SystemPrompt.defaultLayer, LLM.defaultLayer, Bus.layer, CrossSpawnSpawner.defaultLayer))));
...
export const PromptInput = Schema.Struct({
  ...
  parts: Schema.Array(Schema.Union([MessageV2.TextPartInput, MessageV2.FilePartInput, MessageV2.AgentPartInput, MessageV2.SubtaskPartInput]).annotate({
    discriminator: "type"
  }))
}).pipe(withStatics(s => ({
  zod: zod(s)
...
export const CommandInput = Schema.Struct({
  ...
  parts: Schema.optional(Schema.Array(Schema.Union([Schema.Struct({
