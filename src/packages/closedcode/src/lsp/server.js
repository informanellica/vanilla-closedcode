...
import { Archive } from "@/util/archive.js";
import { Process } from "@/util/process.js";
import { which } from "../util/which.js";
import { Module } from "core/util/module";
import { spawn } from "./launch.js";
import { Npm } from "core/npm";
// LSP language servers are NOT auto-downloaded by default — opening a code file
// must not trigger unsolicited network egress (GitHub releases / HashiCorp / npm
// / go / gem). Opt in with CLOSEDCODE_ENABLE_LSP_DOWNLOAD=1 to allow auto-install.
const LSP_DOWNLOAD_ENABLED = ["1", "true"].includes(process.env["CLOSEDCODE_ENABLE_LSP_DOWNLOAD"]) || ["1", "true"].includes(process.env["CLOSEDCODE_ENABLE_TOOL_DOWNLOAD"]);
...
export const Vue = {
  ...
  async spawn(root) {
    ...
    const args = [];
    if (!binary) {
      if (!LSP_DOWNLOAD_ENABLED) return;
...
export const ESLint = {
  ...
  async spawn(root, ctx) {
    ...
    if (!(await Filesystem.exists(serverPath))) {
      ...
      const ok = await Archive.extractZip(zipPath, Global.Path.bin).then(() => true).catch(error => {
        log.error("Failed to extract vscode-eslint archive", {
          error
        });
        return false;
      });
      ...
      if (stats) {
        log.info("removing old eslint installation", {
          path: finalPath
        });
      ...
      log.info("installed VS Code ESLint server", {
        serverPath
      });
    }
...
export const Ty = {
  ...
  async spawn(root) {
    ...
    const potentialVenvPaths = [process.env["VIRTUAL_ENV"], path.join(root, ".venv"), path.join(root, "venv")].filter(p => p !== undefined);
    for (const venvPath of potentialVenvPaths) {
      const isWindows = process.platform === "win32";
      const potentialPythonPath = isWindows ? path.join(venvPath, "Scripts", "python.exe") : path.join(venvPath, "bin", "python");
...
export const ElixirLS = {
  ...
  async spawn(root) {
    ...
    if (!binary) {
      ...
      binary = path.join(Global.Path.bin, "elixir-ls-master", "release", process.platform === "win32" ? "language_server.bat" : "language_server.sh");
      if (!(await Filesystem.exists(binary))) {
        ...
        if (response.body) await Filesystem.writeStream(zipPath, response.body);
        const ok = await Archive.extractZip(zipPath, Global.Path.bin).then(() => true).catch(error => {
          log.error("Failed to extract elixir-ls archive", {
            error
          });
          return false;
        });
        ...
        await Process.run(["mix", "deps.get"], {
          cwd,
          env
        });
        await Process.run(["mix", "compile"], {
          cwd,
          env
        });
        await Process.run(["mix", "elixir_ls.release2", "-o", "release"], {
          cwd,
          env
        });
...
export const Zls = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      let assetName = "";
      let zlsArch = arch;
      if (arch === "arm64") zlsArch = "aarch64";else if (arch === "x64") zlsArch = "x86_64";else if (arch === "ia32") zlsArch = "x86";
      let zlsPlatform = platform;
      if (platform === "darwin") zlsPlatform = "macos";else if (platform === "win32") zlsPlatform = "windows";
      const ext = platform === "win32" ? "zip" : "tar.xz";
      assetName = `zls-${zlsArch}-${zlsPlatform}.${ext}`;
      const supportedCombos = ["zls-x86_64-linux.tar.xz", "zls-x86_64-macos.tar.xz", "zls-x86_64-windows.zip", "zls-aarch64-linux.tar.xz", "zls-aarch64-macos.tar.xz", "zls-aarch64-windows.zip", "zls-x86-linux.tar.xz", "zls-x86-windows.zip"];
      ...
      if (ext === "zip") {
        const ok = await Archive.extractZip(tempPath, Global.Path.bin).then(() => true).catch(error => {
          log.error("Failed to extract zls archive", {
            error
          });
          return false;
        });
...
export const Razor = {
  ...
  async spawn(root) {
    ...
    log.info("using VS Code Razor extension for roslyn-language-server", {
      extension: razor.extension
    });
    return {
      process: spawn(bin, ["--stdio", "--autoLoadProjects", `--razorSourceGenerator=${razor.compiler}`, `--razorDesignTimePath=${razor.targets}`, "--extension", razor.extension], {
        cwd: root
      })
    };
  }
...
async function installRoslynLanguageServer() {
  ...
  if (resolved) {
    log.info(`installed roslyn-language-server`, {
      bin: resolved
    });
    return resolved;
  }
...
async function findVscodeRazorExtension() {
  ...
  for (const root of [...new Set(roots)]) {
    ...
    const candidates = await Promise.all(entries.filter(entry => entry.isDirectory() && entry.name.startsWith("ms-dotnettools.csharp-")).map(async entry => ({
    ...
    for (const entry of candidates.sort((a, b) => b.modified - a.modified).map(candidate => candidate.path)) {
      ...
      if ((await pathExists(result.compiler)) && (await pathExists(result.targets)) && (await pathExists(result.extension))) {
        return result;
      }
    }
  }
...
export const FSharp = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      log.info(`installed fsautocomplete`, {
        bin
      });
    }
    return {
...
export const Clangd = {
  ...
  async spawn(root) {
    ...
    const entries = await fs.readdir(Global.Path.bin, {
      withFileTypes: true
    }).catch(() => []);
    ...
    const asset = assets.find(item => valid(item) && item.name?.endsWith(".zip")) ?? assets.find(item => valid(item) && item.name?.endsWith(".tar.xz")) ?? assets.find(item => valid(item));
    if (!asset?.name || !asset.browser_download_url) {
      log.error("clangd could not match release asset", {
        tag,
        platform
      });
      return;
    ...
    if (!zip && !tar) {
      log.error("clangd encountered unsupported asset", {
        asset: name
      });
      return;
    }
    if (zip) {
      const ok = await Archive.extractZip(archive, Global.Path.bin).then(() => true).catch(error => {
        log.error("Failed to extract clangd archive", {
          error
        });
        return false;
      });
...
export const JDTLS = {
  ...
  root: async (file, ctx) => {
    ...
    const [projectRoot, wrapperRoot, settingsRoot] = await Promise.all([NearestRoot(["pom.xml", "build.gradle", "build.gradle.kts", ".project", ".classpath"], exclusionsForMonorepos)(file, ctx), NearestRoot(gradleMarkers, settingsMarkers)(file, ctx), NearestRoot(settingsMarkers)(file, ctx)]);
  ...
  async spawn(root) {
    ...
    if (!installed) {
      ...
      const tarResult = await run(["tar", "-xzf", archiveName], {
        cwd: distPath
      });
      if (tarResult.code !== 0) {
        log.error("Failed to extract JDTLS", {
          exitCode: tarResult.code,
          stderr: tarResult.stderr.toString()
        });
        return;
      }
      await fs.rm(path.join(distPath, archiveName), {
        force: true
      });
      log.info("JDTLS download and extraction completed");
    }
    const jarFileName = (await fs.readdir(launcherDir).catch(() => [])).find(item => /^org\.eclipse\.equinox\.launcher_.*\.jar$/.test(item))?.trim() ?? "";
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "closedcode-jdtls-data"));
    return {
      process: spawn(java, ["-jar", launcherJar, "-configuration", configFile, "-data", dataDir, "-Declipse.application=org.eclipse.jdt.ls.core.id1", "-Dosgi.bundles.defaultStartLevel=4", "-Declipse.product=org.eclipse.jdt.ls.core.product", "-Dlog.level=ALL", "--add-modules=ALL-SYSTEM", "--add-opens java.base/java.util=ALL-UNNAMED", "--add-opens java.base/java.lang=ALL-UNNAMED"], {
        cwd: root
      })
    };
  }
...
export const KotlinLS = {
  ...
  async spawn(root) {
    ...
    const launcherScript = process.platform === "win32" ? path.join(distPath, "kotlin-lsp.cmd") : path.join(distPath, "kotlin-lsp.sh");
    ...
    if (!installed) {
      ...
      const arch = process.arch;
      let kotlinArch = arch;
      if (arch === "arm64") kotlinArch = "aarch64";else if (arch === "x64") kotlinArch = "x64";
      let kotlinPlatform = platform;
      if (platform === "darwin") kotlinPlatform = "mac";else if (platform === "linux") kotlinPlatform = "linux";else if (platform === "win32") kotlinPlatform = "win";
      ...
      const ok = await Archive.extractZip(archivePath, distPath).then(() => true).catch(error => {
        log.error("Failed to extract Kotlin LS archive", {
          error
        });
        return false;
      });
      ...
      log.info("Installed Kotlin Language Server", {
        path: launcherScript
      });
    }
...
export const LuaLS = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      let assetName = "";
      let lualsArch = arch;
      if (arch === "arm64") lualsArch = "arm64";else if (arch === "x64") lualsArch = "x64";else if (arch === "ia32") lualsArch = "ia32";
      let lualsPlatform = platform;
      if (platform === "darwin") lualsPlatform = "darwin";else if (platform === "linux") lualsPlatform = "linux";else if (platform === "win32") lualsPlatform = "win32";
      ...
      if (ext === "zip") {
        const ok = await Archive.extractZip(tempPath, installDir).then(() => true).catch(error => {
        ...
        const ok = await run(["tar", "-xzf", tempPath, "-C", installDir]).then(result => result.code === 0).catch(error => {
          log.error("Failed to extract lua-language-server archive", {
            error
          });
          return false;
      ...
      if (platform !== "win32") {
        const ok = await fs.chmod(bin, 0o755).then(() => true).catch(error => {
      ...
      log.info(`installed lua-language-server`, {
        bin
      });
    }
    return {
...
export const TerraformLS = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (downloadResponse.body) await Filesystem.writeStream(tempPath, downloadResponse.body);
      const ok = await Archive.extractZip(tempPath, Global.Path.bin).then(() => true).catch(error => {
        log.error("Failed to extract terraform-ls archive", {
          error
        });
        return false;
      });
      ...
      log.info(`installed terraform-ls`, {
        bin
      });
    }
    return {
...
export const TexLab = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (ext === "zip") {
        const ok = await Archive.extractZip(tempPath, Global.Path.bin).then(() => true).catch(error => {
          log.error("Failed to extract texlab archive", {
            error
          });
          return false;
        });
...
export const Tinymist = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (ext === "zip") {
        const ok = await Archive.extractZip(tempPath, Global.Path.bin).then(() => true).catch(error => {
          log.error("Failed to extract tinymist archive", {
            error
          });
          return false;
        });
        ...
        await run(["tar", "-xzf", tempPath, "--strip-components=1"], {
          cwd: Global.Path.bin
        });
      }
      await fs.rm(tempPath, {
