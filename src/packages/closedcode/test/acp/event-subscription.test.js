...
function inProgressText(update) {
...
function toolEvent(sessionId, cwd, opts) {
  const state = opts.status === "running" ? {
    status: "running",
    input: opts.input,
    ...(opts.metadata && {
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "/tmp/closedcode-acp-test";
        const sessionA = await agent.newSession({
          cwd,
          mcpServers: []
        }).then(x => x.sessionId);
        const sessionB = await agent.newSession({
          cwd,
          mcpServers: []
        }).then(x => x.sessionId);
        controller.push({
  ...
  test("does not emit user_message_chunk for live prompt parts", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        expect(sessionUpdates.filter(u => u.sessionId === sessionId).some(u => u.update.sessionUpdate === "user_message_chunk")).toBe(false);
        stop();
      }
    });
  });
  test("keeps concurrent sessions isolated when message.part.delta events are interleaved", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const push = (sessionId, messageID, delta) => {
          controller.push({
            directory: cwd,
            payload: {
              type: "message.part.delta",
  ...
  test("permission.asked events are handled and replied", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        sdk.permission.reply = async params => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        for (const output of ["a", "a", "ab"]) {
          controller.push(toolEvent(sessionId, cwd, {
            callID: "call_1",
            tool: "bash",
            status: "running",
            input,
        ...
        await new Promise(r => setTimeout(r, 20));
        const snapshots = sessionUpdates.filter(u => u.sessionId === sessionId).filter(u => isToolCallUpdate(u.update)).map(u => inProgressText(u.update));
  ...
  test("emits synthetic pending before first running update for any tool", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await new Promise(r => setTimeout(r, 20));
        const types = sessionUpdates.filter(u => u.sessionId === sessionId).map(u => u.update.sessionUpdate).filter(u => u === "tool_call" || u === "tool_call_update");
        expect(types).toEqual(["tool_call", "tool_call_update", "tool_call", "tool_call_update"]);
        const pendings = sessionUpdates.filter(u => u.sessionId === sessionId && u.update.sessionUpdate === "tool_call");
  ...
  test("does not emit duplicate synthetic pending after replayed running tool", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await new Promise(r => setTimeout(r, 20));
        const types = sessionUpdates.filter(u => u.sessionId === sessionId).map(u => u.update).filter(u => "toolCallId" in u && u.toolCallId === "call_1").map(u => u.sessionUpdate).filter(u => u === "tool_call" || u === "tool_call_update");
