...
async function plugin(dir, kinds, opts, themes) {
...
`);
    ...
    const serverJson = parseJsonc(serverText);
    const tuiJson = parseJsonc(tuiText);
...
`);
  ...
  test("writes tui config for oc-themes-only packages", async () => {
    ...
    await fs.mkdir(path.join(target, "themes"), {
      recursive: true
    });
    await writeFile(path.join(target, "themes", "forest.json"), JSON.stringify({
      theme: {
        text: "#fff"
      }
    }, null, 2));
  ...
  test("force replaces version in both server and tui configs", async () => {
    ...
    await writeFile(server, JSON.stringify({
      plugin: ["acme@1.0.0", "other@1.0.0"]
    }, null, 2));
    await writeFile(tui, JSON.stringify({
      plugin: [["acme@1.0.0", {
        mode: "safe"
      }], "other@1.0.0"]
    }, null, 2));
    ...
    expect(tuiJson.plugin).toEqual([["acme@2.0.0", {
      mode: "safe"
    }], "other@1.0.0"]);
  });
