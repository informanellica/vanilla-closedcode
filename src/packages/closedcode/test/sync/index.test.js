...
afterEach(() => {
  Flag.CLOSEDCODE_EXPERIMENTAL_WORKSPACES = original;
});
describe("SyncEvent", () => {
  function setup() {
    SyncEvent.reset();
  ...
  function expectDefect(effect, pattern) {
  ...
  describe("run", () => {
    it.live("inserts event row", provideTmpdirInstance(() => Effect.gen(function* () {
      const {
        Created
      } = setup();
      yield* SyncEvent.use.run(Created, {
        id: "evt_1",
        name: "first"
      });
    ...
    it.live("increments seq per aggregate", provideTmpdirInstance(() => Effect.gen(function* () {
      const {
        Created
      } = setup();
      yield* SyncEvent.use.run(Created, {
    ...
    it.live("uses custom aggregate field from agg()", provideTmpdirInstance(() => Effect.gen(function* () {
      const {
        Sent
      } = setup();
      yield* SyncEvent.use.run(Sent, {
        item_id: "evt_1",
        to: "james"
      });
      const rows = yield* eventRows();
    ...
    it.live("emits events", provideTmpdirInstance(() => Effect.gen(function* () {
      const {
        Created
      } = setup();
      const events = [];
  ...
  describe("replay", () => {
    it.live("inserts event from external payload", provideTmpdirInstance(() => Effect.gen(function* () {
    ...
    it.live("throws on sequence mismatch", provideTmpdirInstance(() => Effect.gen(function* () {
      const id = MessageID.ascending();
      yield* SyncEvent.use.replay({
        id: "evt_1",
        type: "item.created.1",
      ...
      yield* expectDefect(SyncEvent.use.replay({
        id: "evt_1",
        type: "item.created.1",
        seq: 5,
        aggregateID: id,
    ...
    it.live("throws on unknown event type", provideTmpdirInstance(() => Effect.gen(function* () {
      yield* expectDefect(SyncEvent.use.replay({
        id: "evt_1",
    ...
    it.live("replayAll accepts later chunks after the first batch", provideTmpdirInstance(() => Effect.gen(function* () {
      const {
        Created
      } = setup();
      const id = MessageID.ascending();
