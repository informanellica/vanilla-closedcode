...
async function addUser(sessionID, text) {
  const id = MessageID.ascending();
  await svc.updateMessage({
    id,
    sessionID,
...
async function addAssistant(sessionID, parentID, opts) {
  const id = MessageID.ascending();
  await svc.updateMessage({
    id,
    sessionID,
...
async function addCompactionPart(sessionID, messageID, tailStartID) {
  await svc.updatePart({
    id: PartID.ascending(),
    sessionID,
    messageID,
...
describe("MessageV2.page", () => {
  test("resolves with a result", async () => {
    await WithInstance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({});
        await fill(session.id, 2);
        const result = await MessageV2.page({
  ...
  test("throws NotFoundError for non-existent session", async () => {
    await WithInstance.provide({
      directory: root,
      fn: async () => {
        const fake = "non-existent-session";
        await expect(MessageV2.page({
          sessionID: fake,
          limit: 10
  ...
  test("does not return messages from other sessions", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await fill(a.id, 3);
        await fill(b.id, 2);
        const resultA = await MessageV2.page({
          sessionID: a.id,
          limit: 10
        });
        const resultB = await MessageV2.page({
          sessionID: b.id,
          limit: 10
        });
  ...
  test("is an async generator", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        // async generator returns a Promise of { value, done }
  ...
  test("returns message with hydrated parts", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({});
        const [id] = await fill(session.id, 1);
        const result = await MessageV2.get({
          sessionID: session.id,
          messageID: id
        });
        ...
        expect(result.parts[0].text).toBe("m0");
        await svc.remove(session.id);
      }
    });
  });
  ...
  test("returns assistant message with correct role", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        expect(result.parts).toHaveLength(1);
        expect(result.parts[0].text).toBe("response");
        await svc.remove(session.id);
      }
    });
  });
  ...
  test("stops at compaction boundary and returns chronological order", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const a1 = await addAssistant(session.id, u1, {
          summary: true,
          finish: "end_turn"
        });
        await svc.updatePart({
  ...
  test("skips assistant with error even if marked as summary", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await addAssistant(session.id, u1, {
          summary: true,
          finish: "end_turn",
          error
        });
  ...
  test("retains original tail when compaction stores tail_start_id", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const s1 = await addAssistant(session.id, c1, {
          summary: true,
          finish: "end_turn"
        });
        await svc.updatePart({
  ...
  test("retains an assistant tail when compaction starts inside a turn", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const a3 = await addAssistant(session.id, u2, {
          finish: "end_turn"
        });
        await svc.updatePart({
          id: PartID.ascending(),
        ...
        const u3 = await addUser(session.id, "third");
        const a4 = await addAssistant(session.id, u3, {
          finish: "end_turn"
        });
        await svc.updatePart({
          id: PartID.ascending(),
  ...
  test("prefers latest compaction boundary when repeated compactions exist", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const s2 = await addAssistant(session.id, c2, {
          summary: true,
          finish: "end_turn"
        });
        await svc.updatePart({
        ...
        const a4 = await addAssistant(session.id, u4, {
          finish: "end_turn"
        });
        await svc.updatePart({
          id: PartID.ascending(),
...
describe("MessageV2.cursor", () => {
  ...
  test("encoded cursor is base64url", () => {
    const encoded = MessageV2.cursor.encode({
      id: MessageID.ascending(),
      time: 0
    });
  ...
  test("page hydration matches get for each message", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({});
        await fill(session.id, 3);
        const paged = await MessageV2.page({
          sessionID: session.id,
          limit: 10
        });
        for (const item of paged.items) {
          const got = await MessageV2.get({
            sessionID: session.id,
