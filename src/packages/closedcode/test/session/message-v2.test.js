...
function assistantInfo(id, parentID, error, meta) {
  const infoModel = meta ?? {
...
function basePart(messageID, id) {
  return {
    id: PartID.make(id),
    sessionID,
...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("truncates tool output when requested", async () => {
    ...
    expect(await MessageV2.toModelMessages(input, model, {
  ...
  test("forwards partial bash output for aborted tool calls", async () => {
    const userID = "m-user";
    const assistantID = "m-assistant";
    const output = ["31403", "12179", "4575", "", "<shell_metadata>", "User aborted the command", "</shell_metadata>"].join("\n");
    const input = [{
      info: userInfo(userID),
      parts: [{
        ...basePart(userID, "u1"),
      ...
      parts: [{
        ...
        state: {
          status: "error",
          input: {
            command: "for i in {1..20}; do print -- $RANDOM; sleep 1; done"
          },
  ...
  test("filters assistant messages with non-abort errors", async () => {
    const assistantID = "m-assistant";
    const input = [{
      info: assistantInfo(assistantID, "m-parent", new MessageV2.APIError({
        message: "boom",
        isRetryable: true
      }).toObject()),
      parts: [{
  ...
  test("preserves OpenRouter reasoning details through provider transform", async () => {
    ...
    expect(ProviderTransform.message(await MessageV2.toModelMessages(input, openrouterModel), openrouterModel, {})).toStrictEqual([{
      role: "assistant",
      content: [{
        type: "reasoning",
        text: "thinking",
...
describe("session.message-v2.fromError", () => {
  test("serializes context_length_exceeded as ContextOverflowError", () => {
    ...
    const result = MessageV2.fromError(input, {
      providerID
    });
    expect(result).toStrictEqual({
  ...
  test("serializes OpenAI response server_error stream chunks as retryable APIError", () => {
    const body = {
      ...
      error: {
        type: "server_error",
        code: "server_error",
        message: "An error occurred while processing your request. You can retry your request, or contact us through our help center at help.openai.com if the error persists. Please include the request ID req_77eccd008d984bf6bf82d1b2c2b68715 in your message.",
        param: null
      }
    };
  ...
  test("detects context overflow from APICallError provider messages", () => {
    const cases = ["prompt is too long: 213462 tokens > 200000 maximum", "Your input exceeds the context window of this model", "The input token count (1196265) exceeds the maximum number of tokens allowed (1048575)", "Please reduce the length of the messages or completion", "400 status code (no body)", "413 status code (no body)"];
  ...
  test("does not classify 429 no body as context overflow", () => {
    const result = MessageV2.fromError(new APICallError({
  ...
  test("serializes unknown inputs", () => {
    const result = MessageV2.fromError(123, {
      providerID
    });
    expect(result).toStrictEqual({
      name: "UnknownError",
  ...
  test("serializes tagged errors with their message", () => {
    const result = MessageV2.fromError(new Question.RejectedError(), {
      providerID
    });
    expect(result).toStrictEqual({
      name: "UnknownError",
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    ...
    zlibError.code = "ZlibError";
    zlibError.errno = 0;
    zlibError.path = "";
    const result = MessageV2.fromError(zlibError, {
      providerID
    });
    expect(MessageV2.APIError.isInstance(result)).toBe(true);
