...
describe("Session.Info", () => {
  ...
  test("round-trips every optional field", () => {
    const input = {
      ...
      share: {
        url: "https://share.example.com/s/1"
      },
      title: "Full session",
      version: "1.0.0",
      time: {
...
describe("Session input schemas", () => {
  ...
  test("SetTitleInput rejects missing title", () => {
    expect(() => decodeUnknown(Session.SetTitleInput)({
      sessionID
    })).toThrow();
    expect(() => Session.SetTitleInput.zod.parse({
      sessionID
    })).toThrow();
  });
...
describe("SessionRevert.RevertInput", () => {
  ...
  test("messageID is required, partID is optional", () => {
    ...
    expect(() => SessionRevert.RevertInput.zod.parse({
      sessionID
    })).toThrow();
  });
});
...
describe("SessionStatus.Info", () => {
  ...
  test("idle / busy discriminators", () => {
    ...
    expect(SessionStatus.Info.zod.parse({
      type: "idle"
    })).toEqual({
      type: "idle"
    });
...
describe("SessionPrompt input schemas", () => {
  test("LoopInput is just sessionID", () => {
    ...
    expect(SessionPrompt.LoopInput.zod.parse({
      sessionID
    })).toEqual({
      sessionID
    });
  ...
  test("ShellInput requires agent + command", () => {
    ...
    expect(SessionPrompt.ShellInput.zod.parse(input)).toEqual(expected);
    expect(() => decode({
      sessionID
    })).toThrow();
  });
