...
const env = Layer.mergeAll(Session.defaultLayer, SessionRevert.defaultLayer, Snapshot.defaultLayer, CrossSpawnSpawner.defaultLayer);
const it = testEffect(env);
const user = Effect.fn("test.user")(function* (sessionID, agent = "default") {
...
const assistant = Effect.fn("test.assistant")(function* (sessionID, parentID, dir) {
  const session = yield* Session.Service;
  return yield* session.updateMessage({
    id: MessageID.ascending(),
    role: "assistant",
...
const text = Effect.fn("test.text")(function* (sessionID, messageID, content) {
  const session = yield* Session.Service;
...
const tool = Effect.fn("test.tool")(function* (sessionID, messageID) {
  const session = yield* Session.Service;
  return yield* session.updatePart({
    id: PartID.ascending(),
    messageID,
...
const read = file => Effect.promise(() => fs.readFile(file, "utf-8"));
const write = (file, text) => Effect.promise(() => fs.writeFile(file, text));
const tokens = {
  input: 0,
  output: 0,
  reasoning: 0,
...
describe("revert + compact workflow", () => {
  it.live("should properly handle compact command after revert", provideTmpdirInstance(dir => Effect.gen(function* () {
    const session = yield* Session.Service;
    ...
    let messages = yield* session.messages({
      sessionID
    });
  ...
  it.live("should properly clean up revert state before creating compaction message", provideTmpdirInstance(dir => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    const info = yield* session.create({});
    const sessionID = info.id;
  ...
  it.live("cleanup with partID removes parts from the revert point onward", provideTmpdirInstance(() => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    const info = yield* session.create({});
    const sid = info.id;
  ...
  it.live("cleanup removes messages after revert point but keeps earlier ones", provideTmpdirInstance(dir => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    const info = yield* session.create({});
    const sid = info.id;
  ...
  it.live("cleanup is a no-op when session has no revert state", provideTmpdirInstance(() => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    const info = yield* session.create({});
    const sid = info.id;
  ...
  it.live("restore messages in sequential order", provideTmpdirInstance(dir => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    ...
    const info = yield* session.create({});
    const sid = info.id;
    const turn = Effect.fn("test.turn")(function* (file, next) {
  ...
  it.live("restore same file in sequential order", provideTmpdirInstance(dir => Effect.gen(function* () {
    const session = yield* Session.Service;
    const revert = yield* SessionRevert.Service;
    const snapshot = yield* Snapshot.Service;
    yield* write(path.join(dir, "a.txt"), "a0");
    const info = yield* session.create({});
    const sid = info.id;
    const turn = Effect.fn("test.turnSame")(function* (next) {
      const u = yield* user(sid);
