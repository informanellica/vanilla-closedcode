...
import {  SessionRetry  } from "../../src/session/retry.js"
...
function apiError(headers) {
  return MessageV2.APIError.Schema.parse(new MessageV2.APIError({
    message: "boom",
    isRetryable: true,
...
describe("session.retry.delay", () => {
  test("caps delay at 30 seconds when headers missing", () => {
    ...
    const delays = Array.from({
      length: 10
    }, (_, index) => SessionRetry.delay(index + 1, error));
  ...
  test("uses retry-after values even when exceeding 10 minutes with headers", () => {
    ...
    const longError = apiError({
      "retry-after-ms": "700000"
    });
    expect(SessionRetry.delay(1, longError)).toBe(700000);
  });
  test("caps oversized header delays to the runtime timer limit", () => {
    const error = apiError({
      "retry-after-ms": "999999999999"
    });
    expect(SessionRetry.delay(1, error)).toBe(SessionRetry.RETRY_MAX_DELAY);
  });
  it.live("policy updates retry status and increments attempts", () => provideTmpdirInstance(() => Effect.gen(function* () {
...
describe("session.retry.retryable", () => {
  test("maps too_many_requests json messages", () => {
    const error = wrap(JSON.stringify({
      type: "error",
      error: {
  ...
  test("retries plain text rate limit errors from Alibaba", () => {
    const msg = "Upstream error from Alibaba: Request rate increased too quickly. To ensure system stability, please adjust your client logic to scale requests more smoothly over time.";
  ...
  test("retries 500 errors even when isRetryable is false", () => {
    const error = MessageV2.APIError.Schema.parse(new MessageV2.APIError({
...
describe("session.message-v2.fromError", () => {
  test.concurrent("converts ECONNRESET socket errors to retryable APIError", async () => {
    const server = await nodeFetchServer({
      port: 0,
      async fetch(_req) {
    ...
    // Destroy the underlying socket ~100ms after the response starts to mimic
    // Bun.serve's idleTimeout-triggered force-close; the client sees ECONNRESET.
    server.server.on("request", (_req, res) => {
      setTimeout(() => { try { res.socket?.destroy() } catch {} }, 100);
    });
    let error;
    try {
      error = await fetch(new URL("/", server.url.origin)).then(res => res.text()).catch(e => e);
    } finally {
      await server.stop();
    }
    ...
    expect(result.data.metadata?.message).toContain("socket connection");
  }, 15_000);
