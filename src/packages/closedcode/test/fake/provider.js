...
(function (_ProviderTest) {
  ...
  _ProviderTest.model = model;
  function info(override = {}, mdl = model()) {
  ...
  _ProviderTest.info = info;
  function fake(override = {}) {
    const mdl = override.model ?? model();
    const row = override.info ?? info({}, mdl);
    return {
      model: mdl,
      info: row,
      layer: Layer.succeed(Provider.Service, Provider.Service.of({
        ...
        getLanguage: Effect.fn("TestProvider.getLanguage")(() => Effect.die(new Error("ProviderTest.getLanguage not configured"))),
        closest: Effect.fn("TestProvider.closest")(providerID => Effect.succeed(providerID === row.id ? {
          providerID: row.id,
          modelID: mdl.id
        } : undefined)),
        getSmallModel: Effect.fn("TestProvider.getSmallModel")(providerID => Effect.succeed(providerID === row.id ? mdl : undefined)),
        defaultModel: Effect.fn("TestProvider.defaultModel")(() => Effect.succeed({
          providerID: row.id,
          modelID: mdl.id
        })),
        ...override
