...
const testServerLayer = Layer.mergeAll(NodeHttpServer.layer(Http.createServer, {
  host: "127.0.0.1",
  port: 0
}), WorkspaceOld.defaultLayer, SessionNs.defaultLayer);
const it = testEffect(testServerLayer);
const originalWorkspacesFlag = Flag.CLOSEDCODE_EXPERIMENTAL_WORKSPACES;
const originalEnv = {
  CLOSEDCODE_AUTH_CONTENT: process.env.CLOSEDCODE_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.CLOSEDCODE_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag;
  restoreEnv();
  await resetDatabase();
});
...
const createWorkspace = input => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.create(input)));
const restoreWorkspaceSession = input => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.sessionRestore(input)));
const listWorkspaces = project => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.list(project)));
const getWorkspace = id => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.get(id)));
const removeWorkspace = id => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.remove(id)));
const workspaceStatus = () => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.status()));
const isWorkspaceSyncing = id => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.isSyncing(id)));
const startWorkspaceSyncing = projectID => {
  void runWorkspace(WorkspaceOld.Service.use(workspace => workspace.startWorkspaceSyncing(projectID)));
};
const waitForWorkspaceSync = (workspaceID, state, signal) => runWorkspace(WorkspaceOld.Service.use(workspace => workspace.waitForSync(workspaceID, state, signal)));
...
function eventuallyEffect(effect, timeout = 1500) {
...
function localAdapter(dir, input) {
...
function remoteAdapter(url, input) {
  return recordedAdapter({
    configure(info) {
      return {
        ...info,
        directory: input?.directory ?? info.directory
      };
    },
    target() {
      return {
...
function eventStreamResponse(events = [], keepOpen = true) {
...
function workspaceInfo(projectID, type, input) {
  return {
...
describe("workspace-old schemas and exports", () => {
  ...
  test("validates create input with workspace id, project id, branch, type, and extra", () => {
    ...
    expect(() => WorkspaceOld.CreateInput.zod.parse({
      ...input,
      id: "bad"
    })).toThrow();
    expect(() => WorkspaceOld.CreateInput.zod.parse({
  ...
  test("validates session restore input", () => {
    ...
    expect(() => WorkspaceOld.SessionRestoreInput.zod.parse({
      ...input,
      workspaceID: "bad"
    })).toThrow();
    expect(() => WorkspaceOld.SessionRestoreInput.zod.parse({
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async dir => {
      process.env.CLOSEDCODE_AUTH_CONTENT = JSON.stringify({
        test: {
          type: "api",
          key: "secret"
        }
      ...
      expect(JSON.parse(recorded.calls.create[0].env.CLOSEDCODE_AUTH_CONTENT ?? "{}")).toEqual({
        test: {
          type: "api",
          key: "secret"
        }
      });
      expect(recorded.calls.create[0].env.CLOSEDCODE_WORKSPACE_ID).toBe(workspaceID);
      expect(recorded.calls.create[0].env.CLOSEDCODE_EXPERIMENTAL_WORKSPACES).toBe("true");
  ...
  test("create propagates configure failures and does not insert a workspace", async () => {
    await withInstance(async () => {
      ...
      registerAdapter(Instance.project.id, type, recordedAdapter({
        configure() {
  ...
  test("create returns after a local workspace reports error", async () => {
    await withInstance(async dir => {
      ...
      const recorded = localAdapter(missing, {
        createDir: false
      });
      registerAdapter(Instance.project.id, type, recorded.adapter);
      const info = await createWorkspace({
  ...
  it.live("remote create connects to routed event and history endpoints", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(dir => Effect.gen(function* () {
        ...
        expect(calls.map(call => `${call.method} ${call.url.pathname}${call.url.search}${call.url.hash}`)).toEqual(["GET /base/global/event", "POST /base/sync/history"]);
  ...
  test("remove still deletes the row when the adapter cannot remove resources", async () => {
    await withInstance(async () => {
      ...
      const info = workspaceInfo(Instance.project.id, type, {
        id: WorkspaceID.ascending("wrk_remove_throws")
      });
      registerAdapter(Instance.project.id, type, recordedAdapter({
        async remove() {
...
describe("workspace-old sync state", () => {
  ...
  test("duplicate local status updates are suppressed", async () => {
    await withInstance(async dir => {
      ...
      try {
        ...
        expect(captured.events.filter(event => event.workspace === info.id && event.payload.type === WorkspaceOld.Event.Status.type)).toHaveLength(1);
        await removeWorkspace(info.id);
      } finally {
        captured.dispose();
      }
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(() => Effect.gen(function* () {
        ...
        try {
          ...
          expect(captured.events.filter(event => event.workspace === info.id && event.payload.type === WorkspaceOld.Event.Status.type).map(event => event.payload.properties.status)).toEqual(["disconnected", "connecting", "connected"]);
  ...
  it.live("remote connection HTTP failures set error and clear syncing", () => Effect.gen(function* () {
    yield* HttpServer.serveEffect()(Effect.gen(function* () {
      const req = yield* HttpServerRequest.HttpServerRequest;
      if (new URL(req.url, "http://localhost").pathname === "/failed/global/event") return HttpServerResponse.text("nope", {
        status: 503
      });
  ...
  it.live("remote history HTTP failures set error", () => Effect.gen(function* () {
    yield* HttpServer.serveEffect()(Effect.gen(function* () {
      const req = yield* HttpServerRequest.HttpServerRequest;
      const url = new URL(req.url, "http://localhost");
      if (url.pathname === "/history-failed/global/event") return HttpServerResponse.fromWeb(eventStreamResponse([], false));
      if (url.pathname === "/history-failed/sync/history") return HttpServerResponse.text("history failed", {
        status: 500
      });
      return HttpServerResponse.fromWeb(Response.json([]));
    }));
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(() => Effect.gen(function* () {
        ...
        try {
          ...
          expect(captured.events.some(event => event.workspace === info.id && event.payload.type === "sync" && event.payload.syncEvent.seq === historyNextSeq)).toBe(true);
          yield* workspace.remove(info.id);
        } finally {
          captured.dispose();
        }
  ...
  it.live("SSE forwards non-heartbeat events and ignores heartbeats", () => Effect.gen(function* () {
    yield* HttpServer.serveEffect()(Effect.gen(function* () {
      const req = yield* HttpServerRequest.HttpServerRequest;
      const url = new URL(req.url, "http://localhost");
      if (url.pathname === "/sse-forward/global/event") return HttpServerResponse.fromWeb(eventStreamResponse([{
    ...
    yield* provideTmpdirInstance(() => Effect.gen(function* () {
      ...
      try {
        ...
        expect(captured.events.find(event => event.workspace === info.id && event.payload.type === "custom.remote")).toMatchObject({
          directory: "remote-dir",
          project: "remote-project",
          payload: {
            properties: {
  ...
  it.live("SSE sync events are replayed and forwarded", () => {
    ...
    return Effect.gen(function* () {
      yield* HttpServer.serveEffect()(Effect.gen(function* () {
        const req = yield* HttpServerRequest.HttpServerRequest;
        const url = new URL(req.url, "http://localhost");
        if (url.pathname === "/sse-sync/global/event") return HttpServerResponse.fromWeb(eventStreamResponse([{
          directory: "remote-dir",
          project: "remote-project",
          payload: {
            type: "sync",
      ...
      yield* provideTmpdirInstance(() => Effect.gen(function* () {
        ...
        try {
          ...
          expect(captured.events.some(event => event.workspace === info.id && event.payload.type === "sync" && event.payload.syncEvent.seq === sseNextSeq)).toBe(true);
          yield* workspace.remove(info.id);
        } finally {
          captured.dispose();
        }
...
describe("workspace-old waitForSync", () => {
  ...
  test("returns immediately when the stored sequence already satisfies the fence", async () => {
    await withInstance(async () => {
      ...
      await expect(waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_done_2"), {
        [sessionID]: 3
      })).resolves.toBeUndefined();
    });
  });
  ...
  test("rejects with the abort reason when aborted", async () => {
    await withInstance(async () => {
      ...
      const waited = waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_abort"), {
  ...
  test("times out with the requested fence in the error message", async () => {
    await withInstance(async () => {
      ...
      await expect(waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_timeout"), {
        [sessionID]: 1
...
describe("workspace-old sessionRestore", () => {
  ...
  test("throws when switching a missing session fails", async () => {
    await withInstance(async dir => {
      ...
      await expect(restoreWorkspaceSession({
        workspaceID: info.id,
        sessionID: SessionID.descending("ses_missing_restore")
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(dir => Effect.gen(function* () {
        ...
        try {
          ...
          expect(replay.map(call => call.url.pathname + call.url.search + call.url.hash)).toEqual(["/restore/sync/replay", "/restore/sync/replay", "/restore/sync/replay"]);
          ...
          expect(replay.map(call => call.json.events.length)).toEqual([10, 10, 5]);
          expect(replay.map(call => call.json.directory)).toEqual([dir, dir, dir]);
          expect(replay.flatMap(call => call.json.events.map(event => event.seq))).toEqual(Array.from({
            length: 25
          }, (_, i) => i));
          expect(replay[2].json.events.at(-1)).toMatchObject({
            seq: 24,
            type: sessionUpdatedType(),
            data: {
              sessionID: session.id,
  ...
  it.live("remote restore sends an empty directory string when the workspace directory is null", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(() => Effect.gen(function* () {
        ...
        expect(replay[0].json.directory).toBe("");
        expect(replay[0].json.events).toHaveLength(1);
        yield* workspace.remove(info.id);
      }), {
        git: true
      });
  ...
  it.live("remote restore failures include status and body and do not emit completed batch progress", () => {
    ...
    return Effect.gen(function* () {
      yield* HttpServer.serveEffect()(Effect.gen(function* () {
        ...
        return HttpServerResponse.text("replay failed", {
          status: 503
        });
      }));
      const url = yield* serverUrl();
      yield* provideTmpdirInstance(dir => Effect.gen(function* () {
        ...
        try {
          ...
          expect(error.message).toContain(`Failed to replay session ${session.id} into workspace ${info.id}: HTTP 503 replay failed`);
          expect(replay).toHaveLength(1);
          expect(captured.events.filter(event => event.workspace === info.id && event.payload.type === WorkspaceOld.Event.Restore.type).map(event => event.payload.properties.step)).toEqual([0]);
          yield* workspace.remove(info.id);
        } finally {
          captured.dispose();
        }
  ...
  it.live("local restore replays batches and emits progress", () => provideTmpdirInstance(dir => Effect.gen(function* () {
    const workspace = yield* WorkspaceOld.Service;
    const sessionSvc = yield* SessionNs.Service;
    const captured = captureGlobalEvents();
    try {
  ...
  it.live("session restore includes real message and part events in sequence order", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* provideTmpdirInstance(dir => Effect.gen(function* () {
        ...
        const posted = replay[0].json.events;
