...
const scope = Effect.fnUntraced(function* () {
  ...
  yield* Effect.addFinalizer(() => fs.remove(path.join(dir, ...root), {
    recursive: true,
    force: true
  }).pipe(Effect.ignore));
  return {
...
function remappedFs(root) {
  return Layer.effect(AppFileSystem.Service, Effect.gen(function* () {
    const fs = yield* AppFileSystem.Service;
    return AppFileSystem.Service.of({
      ...
      glob: (pattern, options) => fs.glob(pattern, options?.cwd ? {
        ...options,
        cwd: remap(root, options.cwd)
      } : options)
    });
...
const remappedStorage = root => Layer.fresh(Storage.layer.pipe(Layer.provide(remappedFs(root)), Layer.provide(Git.defaultLayer)));
describe("Storage", () => {
  it.live("round-trips JSON content", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
    ...
    expect(yield* svc.read(key)).toEqual(value);
  }));
  it.live("maps missing reads to NotFoundError", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
  ...
  it.live("update on missing key throws NotFoundError", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
    const exit = yield* svc.update([...root, "missing", "key"], draft => {
      draft.value += 1;
    }).pipe(Effect.exit);
    expect(Exit.isFailure(exit)).toBe(true);
  }));
  it.live("write overwrites existing value", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
    ...
    expect(yield* svc.read(key)).toEqual({
      v: 2
    });
  }));
  it.live("remove on missing key is a no-op", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
  ...
  it.live("list on missing prefix returns empty", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
  ...
  it.live("serializes concurrent updates for the same key", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
    ...
    yield* Effect.all(Array.from({
      length: 25
    }, () => svc.update(key, draft => {
      draft.value += 1;
    })), {
  ...
  it.live("concurrent reads do not block each other", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
    ...
    const results = yield* Effect.all(Array.from({
      length: 10
    }, () => svc.read(key)), {
      concurrency: "unbounded"
    });
  ...
  it.live("nested keys create deep paths", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
  ...
  it.live("lists and removes stored entries", () => Effect.gen(function* () {
    const {
      root,
      svc
    } = yield* scope();
  ...
  it.live("migration 2 runs when marker contents are invalid", () => Effect.gen(function* () {
    const fs = yield* AppFileSystem.Service;
    ...
    yield* fs.writeWithDirs(path.join(storage, "session", "proj_test", "ses_test.json"), JSON.stringify({
      id: "ses_test",
      projectID: "proj_test",
      title: "legacy",
      summary: {
    ...
    yield* Effect.gen(function* () {
      ...
      expect(yield* svc.read(["session_diff", "ses_test"])).toEqual(diffs);
      expect(yield* svc.read(["session", "proj_test", "ses_test"])).toEqual({
        id: "ses_test",
        projectID: "proj_test",
        title: "legacy",
        summary: {
  ...
  it.live("migration 1 tolerates malformed legacy records", () => Effect.gen(function* () {
    const fs = yield* AppFileSystem.Service;
    const tmp = yield* tmpdirScoped({
      git: true
    });
    ...
    yield* fs.writeWithDirs(path.join(legacy, "storage", "session", "message", "probe", "1.json"), JSON.stringify({
      path: {
        root: tmp
      }
    }));
    yield* fs.writeWithDirs(path.join(legacy, "storage", "session", "info", "ses_legacy.json"), JSON.stringify({
      id: "ses_legacy",
      title: "legacy"
    }));
    yield* fs.writeWithDirs(path.join(legacy, "storage", "session", "message", "ses_legacy", "msg_legacy.json"), JSON.stringify({
      role: "user",
      text: "hello"
    }));
    yield* Effect.gen(function* () {
      ...
      expect(yield* svc.read(["session", project, "ses_legacy"])).toEqual({
        id: "ses_legacy",
        title: "legacy"
      });
      expect(yield* svc.read(["message", "ses_legacy", "msg_legacy"])).toEqual({
        role: "user",
        text: "hello"
      });
    }).pipe(Effect.provide(remappedStorage(tmp)));
  ...
  it.live("failed migrations do not advance the marker", () => Effect.gen(function* () {
    const fs = yield* AppFileSystem.Service;
    const tmp = yield* tmpdirScoped();
    const storage = path.join(tmp, "storage");
    const legacy = path.join(tmp, "project", "legacy");
