...
The `tmpdir` function in `fixture/fixture.js` creates temporary directories for tests with automatic cleanup.
...
- `config?: Partial<Config.Info>` - Write a `closedcode.json` config file
...
- Directories are created in the system temp folder with prefix `closedcode-test-`
...
Use `testEffect(...)` from `test/lib/effect.js` for tests that exercise Effect services or Effect-based workflows.
...
Prefer the Effect-aware helpers from `fixture/fixture.js` instead of building a manual runtime in each test.
...
Use `it.instance(...)` by default when a test only needs one temp instance. Yield `TestInstance` from `fixture/fixture.js` when the test needs the temp directory path:
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("closedcode-test-")
  }),
)
```
