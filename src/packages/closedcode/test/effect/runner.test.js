...
describe("Runner", () => {
  ...
  it.live("ensureRunning starts work and returns result", Effect.gen(function* () {
  ...
  it.live("ensureRunning propagates work failures", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("concurrent callers share the same run", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("concurrent callers all receive same error", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const work = Effect.gen(function* () {
      yield* Effect.sleep("10 millis");
    ...
    const [a, b] = yield* Effect.all([runner.ensureRunning(work).pipe(Effect.exit), runner.ensureRunning(work).pipe(Effect.exit)], {
      concurrency: "unbounded"
    });
    expect(Exit.isFailure(a)).toBe(true);
    expect(Exit.isFailure(b)).toBe(true);
  }));
  it.live("ensureRunning can be called again after previous run completes", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("second ensureRunning ignores new work if already running", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("cancel interrupts running work", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("cancel on idle is a no-op", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    yield* runner.cancel;
    expect(runner.busy).toBe(false);
  }));
  it.live("cancel with onInterrupt resolves callers gracefully", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s, {
  ...
  it.live("cancel with queued callers resolves all", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s, {
      onInterrupt: Effect.succeed("fallback")
    });
  ...
  it.live("work can be started after cancel", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("cancel does not deadlock when replacement work starts before interrupted run exits", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const hit = yield* Deferred.make();
    const hold = yield* Deferred.make();
    const done = yield* Deferred.make();
    yield* Effect.gen(function* () {
      const runner = Runner.make(s);
      const first = Effect.never.pipe(Effect.onInterrupt(() => Deferred.succeed(hit, undefined)), Effect.ensuring(Deferred.await(hold)), Effect.as("first"));
      ...
      expect(Exit.isFailure(exit)).toBe(true);
    }).pipe(Effect.ensuring(Effect.all([Deferred.succeed(hold, undefined), Deferred.succeed(done, undefined)], {
      discard: true
    }).pipe(Effect.ignore)));
  }));
  ...
  it.live("shell runs exclusively", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("shell rejects when run is active", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const started = yield* Deferred.make();
    const fiber = yield* runner.ensureRunning(Effect.gen(function* () {
  ...
  it.live("shell rejects when another shell is running", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
  ...
  it.live("shell rejects via busy callback and cancel still stops the first shell", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s, {
      busy: () => {
  ...
  it.live("cancel interrupts shell", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const gate = yield* Deferred.make();
  ...
  it.live("cancel does not mask shell defects", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s, {
      onInterrupt: Effect.succeed("interrupted")
    });
    const sh = yield* runner.startShell(Effect.never.pipe(Effect.ensuring(Effect.die("boom")), Effect.as("ignored"))).pipe(Effect.forkChild);
    yield* Effect.sleep("10 millis");
    yield* runner.cancel;
  ...
  it.live("ensureRunning queues behind shell then runs after", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const gate = yield* Deferred.make();
  ...
  it.live("multiple ensureRunning callers share the queued run behind shell", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const calls = yield* Ref.make(0);
    const gate = yield* Deferred.make();
  ...
  it.live("cancel during shell_then_run cancels both", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const sh = yield* runner.startShell(Effect.never.pipe(Effect.as("aborted"))).pipe(Effect.forkChild);
    yield* Effect.sleep("10 millis");
  ...
  it.live("onIdle fires when returning to idle from running", Effect.gen(function* () {
    const s = yield* Scope.Scope;
  ...
  it.live("onIdle fires on cancel", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const count = yield* Ref.make(0);
    const runner = Runner.make(s, {
      onIdle: Ref.update(count, n => n + 1)
  ...
  it.live("onBusy fires when shell starts", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const count = yield* Ref.make(0);
    const runner = Runner.make(s, {
  ...
  it.live("busy is true during run", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const gate = yield* Deferred.make();
  ...
  it.live("busy is true during shell", Effect.gen(function* () {
    const s = yield* Scope.Scope;
    const runner = Runner.make(s);
    const gate = yield* Deferred.make();
