...
const getTuiConfig = async directory => Effect.runPromise(TuiConfig.Service.use(svc => svc.get()).pipe(Effect.provide(TuiConfig.defaultLayer.pipe(Layer.provide(Layer.succeed(CurrentWorkingDirectory, directory))))));
afterEach(async () => {
  ...
  await fs.rm(path.join(Global.Path.config, "tui.jsonc"), {
    force: true
  }).catch(() => {});
  await clear(true);
});
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      ...
      await writeFile(path.join(Global.Path.config, "tui.json"), JSON.stringify({
        plugin: [["shared-plugin@1.0.0", {
          source: "global"
        }], "global-only@1.0.0"]
      }, null, 2));
      ...
      await writeFile(path.join(local, "tui.json"), JSON.stringify({
        plugin: [["shared-plugin@2.0.0", {
          source: "local"
        }], "local-only@1.0.0"]
      }, null, 2));
...
test("migrates legacy tui keys across multiple opencode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      ...
      await writeFile(path.join(nested, "opencode.json"), JSON.stringify({
        theme: "nested-theme"
      }, null, 2));
    }
  });
...
test("project config takes precedence over CLOSEDCODE_TUI_CONFIG (matches CLOSEDCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "tui.json"), JSON.stringify({
        theme: "project",
      ...
      process.env.CLOSEDCODE_TUI_CONFIG = custom;
    }
  });
  const config = await getTuiConfig(tmp.path);
...
test("CLOSEDCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      const custom = path.join(dir, "custom-tui.json");
      await writeFile(custom, JSON.stringify({
...
test("does not derive tui path from CLOSEDCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
...
test("applies env and file substitutions in tui.json", async () => {
  ...
  try {
    await using tmp = await tmpdir({
      init: async dir => {
        await writeFile(path.join(dir, "keybind.txt"), "ctrl+q");
        await writeFile(path.join(dir, "tui.json"), JSON.stringify({
          theme: "{env:TUI_THEME_TEST}",
          keybinds: {
            app_exit: "{file:keybind.txt}"
          }
        }));
      }
    });
    ...
    if (original === undefined) delete process.env.TUI_THEME_TEST;else process.env.TUI_THEME_TEST = original;
  }
});
test("applies file substitutions when first identical token is in a commented line", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "theme.txt"), "resolved-theme");
...
test("supports tuple plugin specs with options in tui.json", async () => {
  ...
  const config = await getTuiConfig(tmp.path);
  expect(config.plugin).toEqual([["acme-plugin@1.2.3", {
    enabled: true,
    label: "demo"
  }]]);
