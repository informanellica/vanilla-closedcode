...
function evalPerm(agent, permission) {
...
test("plan agent denies edits except .closedcode/plans/* (and legacy .opencode/plans/*)", async () => {
  await using tmp = await tmpdir();
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      // Primary .closedcode path is allowed
      expect(Permission.evaluate("edit", ".closedcode/plans/foo.md", plan.permission).action).toBe("allow");
      // Legacy .opencode path is also allowed
...
test("explore agent asks for external directories and allows whitelisted external paths", async () => {
  const {
    Truncate
  } = await import("../../src/tool/truncate.js");
  await using tmp = await tmpdir();
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("external_directory", path.join(Global.Path.tmp, "agent-work"), explore.permission).action).toBe("allow");
    }
  });
});
...
test("global tmp directory children are allowed for external_directory", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      const build = await load(tmp.path, svc => svc.get("build"));
      expect(Permission.evaluate("external_directory", path.join(Global.Path.tmp, "scratch"), build.permission).action).toBe("allow");
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async dir => {
      const skillDir = path.join(dir, ".closedcode", "skill", "perm-skill");
...
`);
  ...
  process.env.CLOSEDCODE_TEST_HOME = tmp.path;
  try {
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const build = await load(tmp.path, svc => svc.get("build"));
        const skillDir = path.join(tmp.path, ".closedcode", "skill", "perm-skill");
...
test("defaultAgent throws when default_agent points to non-existent agent", async () => {
  ...
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      await expect(load(tmp.path, svc => svc.defaultAgent())).rejects.toThrow('default agent "does_not_exist" not found');
    }
  });
});
