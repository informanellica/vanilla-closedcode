...
describe("ProviderTransform.options - google thinkingConfig gating", () => {
  const sessionID = "test-session-123";
  const createGoogleModel = (reasoning, npm) => ({
...
describe("ProviderTransform.providerOptions", () => {
  ...
  test("uses sdk key for non-gateway models", () => {
    ...
    expect(ProviderTransform.providerOptions(model, {
      cachePoint: {
        type: "default"
      }
    })).toEqual({
...
describe("ProviderTransform.schema - gemini combiner nodes", () => {
  ...
  test("does not add sibling keys to combiner nodes during sanitize", () => {
    ...
    walk(result, (node, path) => {
      ...
      const before = path.reduce((acc, key) => acc?.[key], input);
...
describe("ProviderTransform.message - anthropic empty content filtering", () => {
  ...
  test("splits anthropic assistant messages when text trails tool calls", () => {
    const msgs = [{
      ...
      content: [{
        type: "text",
        text: "Check my home directory for PDFs"
      }]
    }, {
      role: "assistant",
      content: [{
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves itemId and reasoningEncryptedContent when store=false even when not openai", () => {
    ...
    const result = ProviderTransform.message(msgs, zenModel, {
      store: false
    });
    expect(result).toHaveLength(1);
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("rs_123");
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const opencodeModel = {
      ...
      api: {
        id: "opencode-test",
        url: "https://opencode-provider.example.invalid",
        npm: "@ai-sdk/openai-compatible"
      }
    };
    const msgs = [{
    ...
    const result = ProviderTransform.message(msgs, opencodeModel, {
      store: false
    });
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [{
      ...
      content: [{
        ...
        providerOptions: {
          openai: {
            itemId: "msg_openai_part"
          },
          opencode: {
            itemId: "msg_opencode_part"
          },
          extra: {
            itemId: "msg_extra_part"
          }
        }
      }]
    }];
...
describe("ProviderTransform.message - providerOptions key remapping", () => {
  const createModel = (providerID, npm) => ({
  ...
  test("azure keeps 'azure' key and does not remap to 'openai'", () => {
    ...
    const result = ProviderTransform.message(msgs, model, {});
    expect(result[0].providerOptions?.azure).toEqual({
      someOption: "value"
    });
  ...
  test("azure cognitive services remaps providerID to 'azure' key", () => {
    ...
    expect(part.providerOptions?.azure).toEqual({
      part: true
    });
...
describe("ProviderTransform.message - claude w/bedrock custom inference profile", () => {
  test("adds cachePoint", () => {
    ...
    const result = ProviderTransform.message(msgs, model, {});
    expect(result[0].providerOptions?.bedrock).toEqual(expect.objectContaining({
      cachePoint: {
        type: "default"
      }
    }));
