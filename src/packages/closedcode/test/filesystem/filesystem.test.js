...
describe("AppFileSystem", () => {
  describe("isDir", () => {
    it("returns true for directories", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
      expect(yield* fs.isDir(tmp)).toBe(true);
    }));
    it("returns false for files", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("returns false for non-existent paths", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      expect(yield* fs.isDir("/tmp/nonexistent-" + Math.random())).toBe(false);
    }));
  });
  describe("isFile", () => {
    it("returns true for files", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
      const file = path.join(tmp, "test.txt");
      yield* fs.writeFileString(file, "hello");
    ...
    it("returns false for directories", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("readJson / writeJson", () => {
    it("round-trips JSON data", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("ensureDir", () => {
    it("creates nested directories", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("is idempotent", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("writeWithDirs", () => {
    it("creates parent directories if missing", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("writes directly when parent exists", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("writes Uint8Array content", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("findUp", () => {
    it("finds target in start directory", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("finds target in parent directories", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("returns empty array when not found", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("up", () => {
    it("finds multiple targets walking up", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("glob", () => {
    it("finds files matching pattern", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
      ...
      const result = yield* fs.glob("*.ts", {
        cwd: tmp
      });
      expect(result.sort()).toEqual(["a.ts", "b.ts"]);
    }));
    it("supports absolute paths", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
      yield* fs.writeFileString(path.join(tmp, "file.txt"), "hello");
      const result = yield* fs.glob("*.txt", {
        cwd: tmp,
        absolute: true
      });
  ...
  describe("globMatch", () => {
    it("matches patterns", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
  ...
  describe("globUp", () => {
    it("finds files walking up directories", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
  ...
  describe("built-in passthrough", () => {
    it("exists works", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
    ...
    it("remove works", Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service;
      const tmp = yield* fs.makeTempDirectoryScoped();
