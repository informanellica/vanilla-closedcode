...
import {  disposeAllInstances, tmpdir, tmpdirScoped  } from "../fixture/fixture.js"
...
const effectIt = testEffect(Layer.mergeAll(testStateLayer, Socket.layerWebSocketConstructorGlobal, servedRoutes.pipe(Layer.provide(Socket.layerWebSocketConstructorGlobal), Layer.provideMerge(NodeHttpServer.layerTest), Layer.provideMerge(NodeServices.layer))));
function app() {
  Flag.CLOSEDCODE_EXPERIMENTAL_HTTPAPI = true;
...
describe("pty HttpApi bridge", () => {
  ...
  testPty("serves PTY JSON routes through experimental Effect routes", async () => {
    ...
    const list = await app().request(PtyPaths.list, {
      headers
    });
    ...
    try {
      ...
      const found = await app().request(PtyPaths.get.replace(":ptyID", info.id), {
        headers
      });
    ...
    const missing = await app().request(PtyPaths.get.replace(":ptyID", info.id), {
      headers
    });
  ...
  (process.platform === "win32" ? effectIt.live.skip : effectIt.live)("serves PTY websocket output and input through Effect routes", () => Effect.gen(function* () {
    ...
    const created = yield* HttpClientRequest.post(PtyPaths.create).pipe(directoryHeader(dir), HttpClientRequest.bodyJson({
      command: "/bin/cat",
      title: "websocket"
    }), Effect.flatMap(HttpClient.execute));
    expect(created.status).toBe(200);
    const info = yield* Schema.decodeUnknownEffect(Pty.Info)(yield* created.json);
    const socket = yield* Socket.makeWebSocket(`${(yield* serverUrl()).replace(/^http/, "ws")}${PtyPaths.connect.replace(":ptyID", info.id)}?cursor=-1&directory=${encodeURIComponent(dir)}`, {
      closeCodeIsError: () => false
    });
    const messages = yield* Queue.unbounded();
    yield* socket.runRaw(message => Queue.offer(messages, typeof message === "string" ? message : new TextDecoder().decode(message))).pipe(Effect.catch(() => Effect.void)).pipe(Effect.forkScoped);
    const write = yield* socket.writer;
    const takeUntil = (expected, seen = "") => Effect.gen(function* () {
    ...
    const removed = yield* HttpClientRequest.delete(PtyPaths.remove.replace(":ptyID", info.id)).pipe(directoryHeader(dir), HttpClient.execute);
