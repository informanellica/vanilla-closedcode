...
function app(backend, input) {
  Flag.CLOSEDCODE_EXPERIMENTAL_HTTPAPI = backend === "httpapi";
...
function client(backend, directory, input) {
...
function capture(request) {
  return call(request).pipe(Effect.map(result => ({
    status: result.response.status,
    data: result.data,
    error: result.error
  })));
}
function expectStatus(request, status) {
  return call(request).pipe(Effect.tap(result => Effect.sync(() => expect(result.response.status).toBe(status))), Effect.asVoid);
}
function firstEvent(open) {
  return Effect.acquireRelease(call(open), events => call(async () => void (await events.stream.return?.(undefined))).pipe(Effect.ignore)).pipe(Effect.flatMap(events => call(() => events.stream.next())), Effect.map(result => result.value));
}
...
function firstPartText(value) {
  return record(array(record(value).parts)[0]).text;
}
function sessionTitles(value) {
  return array(value).map(item => record(item).title).filter(title => typeof title === "string").sort();
}
function resetState() {
...
function withProject(backend, options, run) {
  return Effect.acquireRelease(call(() => tmpdir({
  ...
  })), tmp => call(() => tmp[Symbol.asyncDispose]()).pipe(Effect.ignore)).pipe(Effect.tap(tmp => options.setup?.(tmp.path) ?? Effect.void), Effect.flatMap(tmp => run({
    sdk: client(backend, tmp.path),
    directory: tmp.path
  })));
}
function withStandardProject(backend, run) {
  return withProject(backend, {
    setup: writeStandardFiles
  }, run);
}
function withFakeLlm(backend, run) {
  return Effect.gen(function* () {
    const llm = yield* TestLLMServer;
    return yield* withProject(backend, {
...
function writeStandardFiles(dir) {
  return Effect.all([call(() => writeFile(path.join(dir, "hello.txt"), "hello")), call(() => writeFile(path.join(dir, "needle.ts"), "export const needle = 'sdk-parity'\n"))]).pipe(Effect.asVoid);
}
function seedMessage(directory, sessionID) {
  const id = SessionID.make(sessionID);
  return call(async () => await WithInstance.provide({
    directory,
    fn: () => Effect.runPromise(SessionNs.Service.use(svc => Effect.gen(function* () {
...
describe("HttpApi SDK", () => {
  httpapi("uses the generated SDK for global and control routes", Effect.gen(function* () {
    const sdk = client("httpapi");
    const health = yield* call(() => sdk.global.health());
    const log = yield* call(() => sdk.app.log({
    ...
    expect(yield* firstEvent(() => sdk.global.event({
    ...
    yield* expectStatus(() => sdk.auth.set({
      providerID: "test"
    }), 400);
  }));
  httpapi("uses the generated SDK for safe instance routes", withProject("httpapi", {
    git: false,
    setup: writeStandardFiles
  }, ({
    sdk
  }) => Effect.gen(function* () {
    const file = yield* call(() => sdk.file.read({
      path: "hello.txt"
    }));
    const session = yield* call(() => sdk.session.create({
      title: "sdk"
    }));
    const listed = yield* call(() => sdk.session.list({
      roots: true,
      limit: 10
    }));
    ...
    yield* Effect.all([expectStatus(() => sdk.project.current(), 200), expectStatus(() => sdk.config.get(), 200), expectStatus(() => sdk.config.providers(), 200), expectStatus(() => sdk.find.files({
      query: "hello",
      limit: 10
    }), 200)]);
  })));
  parity("matches generated SDK global and control behavior across backends", backend => Effect.gen(function* () {
    const sdk = client(backend);
    const health = yield* capture(() => sdk.global.health());
    const log = yield* capture(() => sdk.app.log({
      service: "sdk-parity",
      level: "info",
      message: "hello"
    }));
    const invalidAuth = yield* capture(() => sdk.auth.set({
      providerID: "test"
    }));
    return {
      statuses: statuses({
  ...
  parity("matches generated SDK global event stream across backends", backend => firstEvent(() => client(backend).global.event({
    signal: AbortSignal.timeout(1_000)
  })).pipe(Effect.map(event => ({
    type: record(record(event).payload).type
  }))));
  parity("matches generated SDK instance event stream across backends", backend => withStandardProject(backend, ({
    sdk
  }) => firstEvent(() => sdk.event.subscribe(undefined, {
    signal: AbortSignal.timeout(1_000)
  })).pipe(Effect.map(event => ({
    type: record(record(event).payload).type
  })))));
  parity("matches generated SDK basic auth behavior across backends", backend => withStandardProject(backend, ({
    directory
  }) => Effect.gen(function* () {
    const missing = yield* capture(() => client(backend, directory, {
      password: "secret"
    }).file.read({
      path: "hello.txt"
    }));
    const bad = yield* capture(() => client(backend, directory, {
      password: "secret",
      headers: {
    ...
    const good = yield* capture(() => client(backend, directory, {
      password: "secret",
      headers: {
  ...
  parity("matches generated SDK instance read routes across backends", backend => withStandardProject(backend, ({
    sdk,
    directory
  }) => Effect.gen(function* () {
    ...
    const file = yield* capture(() => sdk.file.read({
      path: "hello.txt"
    }));
    const files = yield* capture(() => sdk.file.list({
      path: "."
    }));
    const fileStatus = yield* capture(() => sdk.file.status());
    const findFiles = yield* capture(() => sdk.find.files({
      query: "hello",
      limit: 10
    }));
    const findText = yield* capture(() => sdk.find.text({
      pattern: "sdk-parity"
    }));
    ...
    return {
      ...
      paths: {
        directorySelected: record(paths.data).directory === directory
      },
  ...
  parity("matches generated SDK session lifecycle routes across backends", backend => withStandardProject(backend, ({
    sdk
  }) => Effect.gen(function* () {
    const parent = yield* capture(() => sdk.session.create({
      title: "parent"
    }));
    const parentID = String(record(parent.data).id);
    const child = yield* capture(() => sdk.session.create({
      title: "child",
      parentID
    }));
    const childID = String(record(child.data).id);
    const get = yield* capture(() => sdk.session.get({
      sessionID: parentID
    }));
    const update = yield* capture(() => sdk.session.update({
      sessionID: parentID,
      title: "renamed"
    }));
    const roots = yield* capture(() => sdk.session.list({
      roots: true,
      limit: 10
    }));
    const all = yield* capture(() => sdk.session.list({
      roots: false,
      limit: 10
    }));
    const children = yield* capture(() => sdk.session.children({
      sessionID: parentID
    }));
    const todo = yield* capture(() => sdk.session.todo({
      sessionID: parentID
    }));
    ...
    const missingGet = yield* capture(() => sdk.session.get({
      sessionID: "ses_missing"
    }));
    const missingMessages = yield* capture(() => sdk.session.messages({
      sessionID: "ses_missing",
      limit: 2
    }));
    const invalidCursor = yield* capture(() => sdk.session.messages({
      sessionID: parentID,
      limit: 2,
      before: "bad"
    }));
    const deleted = yield* capture(() => sdk.session.delete({
      sessionID: childID
    }));
    const getDeleted = yield* capture(() => sdk.session.get({
      sessionID: childID
    }));
    return {
      statuses: statuses({
  ...
  parity("matches generated SDK session message and part routes across backends", backend => withStandardProject(backend, ({
    sdk,
    directory
  }) => Effect.gen(function* () {
    const session = yield* capture(() => sdk.session.create({
      title: "messages"
    }));
    const sessionID = String(record(session.data).id);
    const seeded = yield* seedMessage(directory, sessionID);
    const list = yield* capture(() => sdk.session.messages({
      sessionID
    }));
    const page = yield* capture(() => sdk.session.messages({
      sessionID,
      limit: 1
    }));
    const message = yield* capture(() => sdk.session.message({
      sessionID,
      messageID: seeded.message.id
    }));
    const partUpdate = yield* capture(() => sdk.part.update({
      sessionID,
      messageID: seeded.message.id,
    ...
    const updated = yield* capture(() => sdk.session.message({
      sessionID,
      messageID: seeded.message.id
    }));
    const partDelete = yield* capture(() => sdk.part.delete({
      sessionID,
      messageID: seeded.message.id,
      partID: seeded.part.id
    }));
    const withoutPart = yield* capture(() => sdk.session.message({
      sessionID,
      messageID: seeded.message.id
    }));
    const deleteMessage = yield* capture(() => sdk.session.deleteMessage({
      sessionID,
      messageID: seeded.message.id
    }));
    const missingMessage = yield* capture(() => sdk.session.message({
      sessionID,
      messageID: seeded.message.id
    }));
    return {
  ...
  parity("matches generated SDK prompt no-reply routes across backends", backend => withStandardProject(backend, ({
    sdk
  }) => Effect.gen(function* () {
    const session = yield* capture(() => sdk.session.create({
      title: "prompt"
    ...
    const sessionID = String(record(session.data).id);
    const prompt = yield* capture(() => sdk.session.prompt({
      sessionID,
      agent: "build",
      noReply: true,
      parts: [{
    ...
    const asyncPrompt = yield* capture(() => sdk.session.promptAsync({
      sessionID,
      agent: "build",
      noReply: true,
      parts: [{
    ...
    return {
      ...
      messageCount: array(messages.data).length,
      messageTexts: array(messages.data).flatMap(item => array(record(item).parts)).map(part => record(part).text).filter(text => typeof text === "string").sort()
    };
  })));
  parity("matches generated SDK prompt streaming through fake LLM across backends", backend => withFakeLlm(backend, ({
    sdk,
    llm
  }) => Effect.gen(function* () {
    yield* llm.text("fake world", {
      usage: {
        input: 11,
        output: 7
      }
  ...
  parity("matches generated SDK TUI validation and command routes across backends", backend => withStandardProject(backend, ({
    sdk
  }) => Effect.gen(function* () {
    const session = yield* capture(() => sdk.session.create({
      title: "tui"
    ...
    const sessionID = String(record(session.data).id);
    const appendPrompt = yield* capture(() => sdk.tui.appendPrompt({
      text: "hello"
    }));
    ...
    const executeCommand = yield* capture(() => sdk.tui.executeCommand({
      command: "session_new"
    }));
    const showToast = yield* capture(() => sdk.tui.showToast({
      title: "SDK",
      message: "hello",
      variant: "info"
    }));
    const selectSession = yield* capture(() => sdk.tui.selectSession({
      sessionID
    }));
    const missingSession = yield* capture(() => sdk.tui.selectSession({
      sessionID: "ses_missing"
    }));
    const invalidSession = yield* capture(() => sdk.tui.selectSession({
  ...
  parity("matches generated SDK project git initialization across backends", backend => withProject(backend, {
    git: false
  }, ({
    sdk,
    directory
