...
import {  TuiEvent  } from "../../src/cli/cmd/tui/event.js"
import {  SessionID  } from "../../src/session/schema.js"
import {  TuiApi, TuiPaths  } from "../../src/server/routes/instance/httpapi/groups/tui.js"
import {  callTui  } from "../../src/server/routes/express/tui.js"
...
async function expectTrue(path, headers, body) {
...
describe("tui HttpApi bridge", () => {
  ...
  test("serves TUI command and event routes through experimental Effect routes", async () => {
    ...
    await expectTrue(TuiPaths.appendPrompt, headers, {
      text: "hello"
    });
    ...
    await expectTrue(TuiPaths.executeCommand, headers, {
      command: "agent_cycle"
    });
    await expectTrue(TuiPaths.showToast, headers, {
      message: "Saved",
      variant: "success"
    });
  ...
  test("matches legacy unknown execute command behavior", async () => {
    ...
    const legacy = await app(false).request(TuiPaths.executeCommand, {
      method: "POST",
      headers,
      body
    });
    ...
    const effect = await app().request(TuiPaths.executeCommand, {
      method: "POST",
      headers,
      body
    });
  ...
  test("serves TUI control queue through experimental Effect routes", async () => {
    ...
    const next = await app().request(TuiPaths.controlNext, {
      headers
    });
    ...
    await expectTrue(TuiPaths.controlResponse, headers, {
      ok: true
    });
