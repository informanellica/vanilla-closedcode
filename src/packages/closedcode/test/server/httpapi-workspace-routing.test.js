...
const it = testEffect(Layer.mergeAll(testStateLayer, NodeHttpServer.layerTest, NodeServices.layer, Project.defaultLayer, Workspace.defaultLayer, Socket.layerWebSocketConstructorGlobal));
const workspaceRoutingTestLayer = workspaceRouterMiddleware.layer.pipe(Layer.provide([Socket.layerWebSocketConstructorGlobal, FetchHttpClient.layer]));
const serverUrl = HttpServer.HttpServer.use(server => Effect.succeed(HttpServer.formatAddress(server.address)));
const requestURL = request => new URL(request.url, "http://localhost");
const listenAdditionalServer = handler => Effect.gen(function* () {
...
const remoteAdapter = (directory, url, headers) => ({
  name: "Remote Test",
...
const eventStreamResponse = () => HttpServerResponse.text('data: {"payload":{"type":"server.connected","properties":{}}}\n\n', {
  contentType: "text/event-stream"
});
const syncResponse = request => {
...
const createWorkspace = input => Effect.acquireRelease(Effect.gen(function* () {
...
const createLocalWorkspace = input => createWorkspace({
  projectID: input.projectID,
  type: input.type,
...
const listenRemoteWebSocket = () => listenAdditionalServer(request => {
  const sync = syncResponse(request);
  if (sync) return sync;
  if (requestURL(request).pathname !== "/base/probe") return Effect.succeed(HttpServerResponse.empty({
    status: 404
  }));
  return echoWebSocket(request);
});
const echoWebSocket = request => Effect.gen(function* () {
...
const serveRouteContextProbe = HttpRouter.add("GET", "/probe", Effect.gen(function* () {
...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () => Effect.gen(function* () {
    ...
    yield* HttpRouter.add("PATCH", "/probe", HttpServerResponse.text("route called")).pipe(Layer.provide(workspaceRoutingTestLayer), HttpRouter.serve, Layer.build);
    const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(HttpClientRequest.setHeaders({
  ...
  it.live("returns 503 when a remote workspace is not actively syncing", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const project = yield* Project.use.fromDirectory(dir);
    ...
    yield* HttpRouter.add("GET", "/probe", HttpServerResponse.text("route called")).pipe(Layer.provide(workspaceRoutingTestLayer), HttpRouter.serve, Layer.build);
  ...
  it.live("proxies remote workspace WebSocket requests through the selected workspace target", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const project = yield* Project.use.fromDirectory(dir);
  ...
  it.live("returns a missing workspace response for unknown workspace ids", () => Effect.gen(function* () {
  ...
  it.live("keeps control-plane routes local even when workspace is selected", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const project = yield* Project.use.fromDirectory(dir);
    ...
    yield* HttpRouter.add("GET", "/session", Effect.gen(function* () {
      const route = yield* WorkspaceRouteContext;
      return yield* HttpServerResponse.json({
        directory: route.directory,
        workspaceID: route.workspaceID
  ...
  it.live("keeps workspace control routes local even when workspace is selected", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const project = yield* Project.use.fromDirectory(dir);
    ...
    yield* HttpRouter.add("GET", WorkspacePaths.list, Effect.gen(function* () {
      const route = yield* WorkspaceRouteContext;
      return yield* HttpServerResponse.json({
        directory: route.directory,
        workspaceID: route.workspaceID
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped();
    ...
    const headerResponse = yield* HttpClientRequest.get("/probe").pipe(HttpClientRequest.setHeader("x-opencode-directory", headerDir), HttpClient.execute);
  ...
  it.live("routes local workspace requests through WorkspaceRouteContext", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const project = yield* Project.use.fromDirectory(dir);
