...
import {  ShellTool  } from "../../src/tool/shell.js"
...
const runtime = ManagedRuntime.make(Layer.mergeAll(CrossSpawnSpawner.defaultLayer, AppFileSystem.defaultLayer, Plugin.defaultLayer, Truncate.defaultLayer, Config.defaultLayer, Agent.defaultLayer));
function initBash() {
...
const shells = (() => {
  ...
  const list = [bash, which("pwsh"), which("powershell"), process.env.COMSPEC || which("cmd.exe")].filter(shell => Boolean(shell)).map(shell => ({
    label: Shell.name(shell),
    shell
  }));
  return list.filter((item, i) => list.findIndex(other => other.shell.toLowerCase() === item.shell.toLowerCase()) === i);
})();
...
const evalarg = text => sh() === "cmd" ? quote(text) : squote(text);
const fill = (mode, n) => {
  const code = mode === "lines" ? "console.log(Array.from({length:Number(process.argv[1])},(_,i)=>i+1).join(String.fromCharCode(10)))" : "process.stdout.write(String.fromCharCode(97).repeat(Number(process.argv[1])))";
...
const each = (name, fn) => {
  for (const item of shells) {
    test(`${name} [${item.label}]`, withShell(item, () => fn(item)));
  }
};
const capture = (requests, stop) => ({
  ...ctx,
  ask: req => Effect.sync(() => {
    requests.push(req);
    if (stop) throw stop;
...
const mustTruncate = result => {
  ...
  throw new Error([`shell: ${process.env.SHELL || ""}`, `exit: ${String(result.metadata.exit)}`, "output:", result.output].join("\n"));
};
describe("tool.shell", () => {
  each("basic", async () => {
...
describe("tool.shell permissions", () => {
  ...
  for (const item of ps) {
    test(`parses PowerShell conditionals for permission prompts [${item.label}]`, withShell(item, async () => {
      await WithInstance.provide({
        directory: projectRoot,
        fn: async () => {
          const bash = await initShell();
  ...
  for (const item of ps) {
    test(`uses PowerShell cmdlet prefixes for always-allow prompts [${item.label}]`, withShell(item, async () => {
      await using tmp = await tmpdir();
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          const requests = [];
          await expect(Effect.runPromise(bash.execute({
            command: "Remove-Item -Recurse tmp",
            description: "Remove a temp directory"
          }, capture(requests, err)))).rejects.toThrow(err.message);
          const bashReq = requests.find(r => r.permission === "bash");
          expect(bashReq).toBeDefined();
  ...
  if (process.platform === "win32") {
    if (bash) {
      test("asks for nested bash command permissions [bash]", withShell({
        label: "bash",
        shell: bash
      }, async () => {
        await using outerTmp = await tmpdir({
          init: async dir => {
            await writeFile(path.join(dir, "outside.txt"), "x");
          }
        });
        await WithInstance.provide({
          directory: projectRoot,
  ...
  if (process.platform === "win32") {
    for (const item of ps) {
      test(`asks for external_directory permission for PowerShell paths after switches [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initShell();
    ...
    for (const item of ps) {
      test(`asks for nested PowerShell command permissions [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initShell();
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for drive-relative PowerShell paths [${item.label}]`, withShell(item, async () => {
        await using tmp = await tmpdir();
        await WithInstance.provide({
          directory: tmp.path,
          fn: async () => {
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for $HOME PowerShell paths [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initShell();
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for $PWD PowerShell paths [${item.label}]`, withShell(item, async () => {
        await using tmp = await tmpdir();
        await WithInstance.provide({
          directory: tmp.path,
          fn: async () => {
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for $PSHOME PowerShell paths [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initBash();
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for missing PowerShell env paths [${item.label}]`, withShell(item, async () => {
        const key = "CLOSEDCODE_TEST_MISSING";
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for PowerShell env paths [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initBash();
    ...
    for (const item of ps) {
      test(`asks for external_directory permission for PowerShell FileSystem paths [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initBash();
            ...
            expect(requests[0]?.permission).toBe("external_directory");
            if (requests[0]?.permission !== "external_directory") return;
            expect(requests[0].patterns).toContain(Filesystem.normalizePathPattern(path.join(process.env.WINDIR, "*")));
          }
        });
      }));
    }
    for (const item of ps) {
      test(`asks for external_directory permission for braced PowerShell env paths [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initBash();
    ...
    for (const item of ps) {
      test(`treats Set-Location like cd for permissions [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initBash();
    ...
    for (const item of ps) {
      test(`does not add nested PowerShell expressions to permission prompts [${item.label}]`, withShell(item, async () => {
        await WithInstance.provide({
          directory: projectRoot,
          fn: async () => {
            const bash = await initShell();
  ...
  if (process.platform === "win32" && cmdShell) {
    test("asks for external_directory permission for cmd file commands [cmd]", withShell(cmdShell, async () => {
      await WithInstance.provide({
        directory: projectRoot,
        fn: async () => {
          const bash = await initShell();
  ...
  if (process.platform === "win32") {
    ...
    if (bash) {
      test("uses Git Bash /tmp semantics for external workdir", withShell({
        label: "bash",
        shell: bash
      }, async () => {
        await WithInstance.provide({
      ...
      test("uses Git Bash /tmp semantics for external file paths", withShell({
        label: "bash",
        shell: bash
      }, async () => {
        await WithInstance.provide({
  ...
  each("does not ask for external_directory permission when rm inside project", async () => {
    await using tmp = await tmpdir({
      init: async dir => {
        await writeFile(path.join(dir, "tmpfile"), "x");
      }
    });
    await WithInstance.provide({
      directory: tmp.path,
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await expect(Effect.runPromise(bash.execute({
          ...
          description: "Redirect test output"
        }, capture(requests, err)))).rejects.toThrow(err.message);
        const bashReq = requests.find(r => r.permission === "bash");
        expect(bashReq).toBeDefined();
