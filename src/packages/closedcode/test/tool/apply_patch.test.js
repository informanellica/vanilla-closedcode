...
describe("tool.apply_patch freeform", () => {
  test("requires patchText", async () => {
    ...
    await expect(execute({
      patchText: ""
    }, ctx)).rejects.toThrow("patchText is required");
  });
  test("rejects invalid patch format", async () => {
    ...
    await expect(execute({
      patchText: "invalid patch"
    }, ctx)).rejects.toThrow("apply_patch verification failed");
  });
  test("rejects empty patch", async () => {
    ...
    await expect(execute({
      patchText: emptyPatch
    }, ctx)).rejects.toThrow("patch rejected: empty patch");
  });
  ...
  test("permission metadata includes move file info", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await fs.mkdir(path.dirname(original), {
          recursive: true
        });
  ...
  test("rejects update when target file is missing", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await expect(execute({
          patchText
        }, ctx)).rejects.toThrow("apply_patch verification failed: Failed to read file to update");
      }
    });
  });
