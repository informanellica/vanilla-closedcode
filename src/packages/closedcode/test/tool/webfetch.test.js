...
async function withFetch(fetch, fn) {
  const server = await nodeFetchServer({ port: 0, fetch });
  try { await fn(server.url); } finally { await server.stop(); }
}
function exec(args) {
  return WebFetchTool.pipe(Effect.flatMap(info => info.init()), Effect.flatMap(tool => tool.execute(args, ctx)), Effect.provide(Layer.mergeAll(FetchHttpClient.layer, Truncate.defaultLayer, Agent.defaultLayer)), Effect.runPromise);
}
describe("tool.webfetch", () => {
  test("returns image responses as file attachments", async () => {
    const bytes = new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]);
    await withFetch(() => new Response(bytes, {
      status: 200,
      headers: {
  ...
  test("keeps svg as text output", async () => {
    ...
    await withFetch(() => new Response(svg, {
      status: 200,
      headers: {
  ...
  test("keeps text responses as text output", async () => {
    await withFetch(() => new Response("hello from webfetch", {
      status: 200,
      headers: {
