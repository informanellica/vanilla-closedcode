...
describe("Git", () => {
  ...
  test("status() handles special filenames", async () => {
    ...
    await withGit(async rt => {
      ...
      expect(status).toEqual(expect.arrayContaining([expect.objectContaining({
        file: weird,
        status: "added"
      })]));
    });
  ...
  test("diff(), stats(), and mergeBase() parse tracked changes", async () => {
    ...
    await withGit(async rt => {
      const [base, diff, stats] = await Promise.all([rt.runPromise(Git.Service.use(git => git.mergeBase(tmp.path, "main"))), rt.runPromise(Git.Service.use(git => git.diff(tmp.path, "HEAD"))), rt.runPromise(Git.Service.use(git => git.stats(tmp.path, "HEAD")))]);
      expect(base).toBeTruthy();
      expect(diff).toEqual(expect.arrayContaining([expect.objectContaining({
        file: weird,
        status: "modified"
      })]));
      expect(stats).toEqual(expect.arrayContaining([expect.objectContaining({
        file: weird,
        additions: 1,
        deletions: 1
      })]));
