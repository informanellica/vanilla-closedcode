...
const TestEvent = {
  Ping: BusEvent.define("test.ping", Schema.Struct({
    value: Schema.Number
  })),
  Pong: BusEvent.define("test.pong", Schema.Struct({
  ...
  describe("publish + subscribe", () => {
    test("subscriber is live immediately after subscribe returns", async () => {
      ...
      await withInstance(tmp.path, async () => {
        ...
        await Bus.publish(TestEvent.Ping, {
          value: 42
        });
        await sleep(10);
      });
    ...
    test("subscriber does not receive events of other types", async () => {
      ...
      await withInstance(tmp.path, async () => {
        ...
        await sleep(10);
        await Bus.publish(TestEvent.Pong, {
          message: "hello"
        });
        await Bus.publish(TestEvent.Ping, {
          value: 1
