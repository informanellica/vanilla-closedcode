import {  offsetToPosition, resolveZedDbPath, resolveZedSelection  } from "../../../src/cli/cmd/tui/context/editor-zed.js"
import {  tmpdir  } from "../../fixture/fixture.js"
import {  DatabaseSync  } from "node:sqlite"
// Bun.sqlite-compatible thin wrapper: .run(sql, params) → prepare + run
function Database(p) {
  ...
  return {
    run(sql, params) {
      ...
      else if (params === undefined) stmt.run();
...
async function writeZedFixture(dir, options = {}) {
  ...
  if (options.editor !== false) {
    ...
    }]).forEach(selection => db.run("insert into editor_selections values (1, 1, ?, ?)", [selection.start, selection.end]));
  }
  db.close();
  return {
    dbPath,
...
function utf8ByteOffset(text, offset) {
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const home = jest.spyOn(os, "homedir").mockImplementation(() => tmp.path);
  const previous = process.env.CLOSEDCODE_ZED_DB;
  process.env.CLOSEDCODE_ZED_DB = loop;
  try {
    expect(resolveZedDbPath()).toBeUndefined();
  } finally {
    if (previous === undefined) delete process.env.CLOSEDCODE_ZED_DB;else process.env.CLOSEDCODE_ZED_DB = previous;
    home.mockRestore();
  }
});
