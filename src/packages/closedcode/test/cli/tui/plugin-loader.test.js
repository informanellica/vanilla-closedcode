...
const {
  allThemes,
  addTheme
} = await import("../../../src/cli/cmd/tui/context/theme.js");
const {
  TuiPluginRuntime
} = await import("../../../src/cli/cmd/tui/plugin/runtime.js");
async function row(file) {
  return Filesystem.readJson(file);
}
async function load() {
  const stamp = Date.now();
  const globalConfigPath = path.join(Global.Path.config, "tui.json");
  const backup = await readText(globalConfigPath).catch(() => undefined);
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "package.json"), JSON.stringify({ type: "module" }));
      const localPluginPath = path.join(dir, "local-plugin.js");
      const invalidPluginPath = path.join(dir, "invalid-plugin.js");
      const preloadedPluginPath = path.join(dir, "preloaded-plugin.js");
      const globalPluginPath = path.join(dir, "global-plugin.js");
      ...
      await writeFile(localThemePath, JSON.stringify({
        theme: {
          primary: "#101010"
        }
      }, null, 2));
      await writeFile(invalidThemePath, "{ invalid json }");
      await writeFile(globalThemePath, JSON.stringify({
        theme: {
          primary: "#202020"
        }
      }, null, 2));
      await writeFile(preloadedThemePath, JSON.stringify({
        theme: {
          primary: "#f0f0f0"
        }
      }, null, 2));
      await writeFile(preloadedDest, JSON.stringify({
        theme: {
          primary: "#303030"
        }
      }, null, 2));
      await writeFile(localPluginPath, `export const ignored = async (_input, options) => {
  if (!options?.fn_marker) return
  await (await import("node:fs/promises")).writeFile(options.fn_marker, "called")
}
...
export default {
  ...
  tui: async (api, options) => {
    ...
    const first = await (await import("node:fs/promises")).readFile(options.dest, "utf8")
    await (await import("node:fs/promises")).writeFile(options.source, JSON.stringify({ theme: { primary: "#fefefe" } }, null, 2))
    await api.theme.install(options.theme_path)
    const second = await (await import("node:fs/promises")).readFile(options.dest, "utf8")
...
`);
      await writeFile(preloadedPluginPath, `export default {
  id: "demo.preloaded",
  tui: async (api, options) => {
    if (!options?.marker) return
    const before = api.theme.has(options.theme_name)
    await api.theme.install(options.theme_path)
    const after = api.theme.has(options.theme_name)
    const text = await (await import("node:fs/promises")).readFile(options.dest, "utf8")
    await (await import("node:fs/promises")).writeFile(
      options.marker,
      JSON.stringify({
        before,
...
`);
      await writeFile(globalPluginPath, `export default {
  id: "demo.global",
  tui: async (api, options) => {
    if (!options?.marker) return
    await api.theme.install(options.theme_path)
...
`);
  ...
  try {
    expect(addTheme(tmp.extra.preloadedThemeName, {
      theme: {
        primary: "#303030"
      }
    })).toBe(true);
    ...
    const config = {
      plugin: [[tmp.extra.localSpec, localOpts], [tmp.extra.invalidSpec, invalidOpts], [tmp.extra.preloadedSpec, preloadedOpts], [tmp.extra.globalSpec, globalOpts]],
      plugin_origins: [{
    ...
    const fn_called = await fs.readFile(tmp.extra.fnMarker, "utf8").then(() => true).catch(() => false);
    ...
    const leaked_local_to_global = await fs.stat(path.join(Global.Path.config, "themes", tmp.extra.localThemeFile)).then(() => true).catch(() => false);
    const leaked_global_to_local = await fs.stat(path.join(tmp.path, ".opencode", "themes", tmp.extra.globalThemeFile)).then(() => true).catch(() => false);
    return {
      local,
      global,
      invalid,
    ...
    cwd.mockRestore();
    if (backup === undefined) {
      await fs.rm(globalConfigPath, {
        force: true
      });
    } else {
      await writeFile(globalConfigPath, backup);
    }
    await fs.rm(tmp.extra.globalDest, {
      force: true
    }).catch(() => {});
  }
}
test("continues loading when a plugin is missing config metadata", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "package.json"), JSON.stringify({ type: "module" }));
      const bad = path.join(dir, "missing-meta-plugin.js");
      const good = path.join(dir, "next-plugin.js");
      const bare = path.join(dir, "plain-plugin.js");
      ...
      for (const [file, id] of [[bad, "demo.missing-meta"], [good, "demo.next"]]) {
...
`);
      ...
      await writeFile(bare, `export default {
  id: "demo.plain",
  tui: async (_api, options) => {
    await (await import("node:fs/promises")).writeFile(${JSON.stringify(bareMarker)}, options === undefined ? "undefined" : "value")
  },
}
`);
      return {
  ...
  const config = {
    plugin: [[tmp.extra.badSpec, {
      marker: path.join(tmp.path, "bad.txt")
    }], [tmp.extra.goodSpec, {
      marker: tmp.extra.goodMarker
    }], tmp.extra.bareSpec],
    plugin_origins: [{
...
test("initializes external tui plugins in config order", async () => {
  ...
  const backupJson = await readText(globalJson).catch(() => undefined);
  const backupJsonc = await readText(globalJsonc).catch(() => undefined);
  await fs.rm(globalJson, {
    force: true
  }).catch(() => {});
  await fs.rm(globalJsonc, {
    force: true
  }).catch(() => {});
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "package.json"), JSON.stringify({ type: "module" }));
      const a = path.join(dir, "order-a.js");
      const b = path.join(dir, "order-b.js");
...
`);
  ...
  const cwd = jest.spyOn(process, "cwd").mockImplementation(() => tmp.path);
  try {
    const a = path.join(tmp.path, "order-a.js");
    const b = path.join(tmp.path, "order-b.js");
    const aSpec = pathToFileURL(a).href;
    const bSpec = pathToFileURL(b).href;
    const config = {
      plugin: [aSpec, bSpec],
    ...
    if (backupJson === undefined) {
      ...
      await writeFile(globalJson, backupJson);
    }
    if (backupJsonc === undefined) {
      ...
      await writeFile(globalJsonc, backupJsonc);
    }
  }
});
describe("tui.plugin.loader", () => {
  let data;
  beforeAll(async () => {    data = await load();});
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      await writeFile(path.join(dir, "package.json"), JSON.stringify({ type: "module" }));
      const pluginPath = path.join(dir, "theme-update-plugin.js");
...
`);
  ...
  const cwd = jest.spyOn(process, "cwd").mockImplementation(() => tmp.path);
  const wait = useWaitForDependencies();
  const mkApi = () => createTuiPluginApi({
    theme: {
      has(name) {
        return allThemes()[name] !== undefined;
      }
  ...
  try {
    ...
    await writeFile(tmp.extra.themePath, JSON.stringify({
      theme: {
        primary: "#222222"
      }
    }, null, 2));
    await writeFile(tmp.extra.pluginPath, `export default {
  id: "demo.theme-update",
  tui: async (api, options) => {
    if (!options?.theme_path) return
    await api.theme.install(options.theme_path)
...
`);
    ...
    const list = await Filesystem.readJson(process.env.CLOSEDCODE_PLUGIN_META_FILE);
