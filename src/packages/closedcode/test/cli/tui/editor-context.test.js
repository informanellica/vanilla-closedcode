...
import {  EditorContextProvider, useEditorContext  } from "../../../src/cli/cmd/tui/context/editor.js"
import {  tmpdir  } from "../../fixture/fixture.js"
import {  FakeWebSocket  } from "../../lib/websocket.js"
import {  mkdir, writeFile  } from "node:fs/promises"
import os from "node:os";
import path from "node:path";
import {  afterEach, expect, test, beforeAll, jest  } from "@jest/globals"
const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT;
const originalOpencodePort = process.env.CLOSEDCODE_EDITOR_SSE_PORT;
afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort;
  process.env.CLOSEDCODE_EDITOR_SSE_PORT = originalOpencodePort;
});
function nextTick() {
  return new Promise(resolve => queueMicrotask(resolve));
}
function mountEditorContext(WebSocketImpl) {
  let editor;
  let dispose;
  createRoot(nextDispose => {
    ...
    return _$createComponent(EditorContextProvider, {
      ...
      get children() {
        return _$createComponent(Consumer, {});
      }
    });
  });
  return {
...
function createWebSocketImpl(...sockets) {
  let index = 0;
  return class {
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  await mkdir(ideDirectory, {
    recursive: true
  });
  await writeFile(path.join(ideDirectory, "3001.lock"), JSON.stringify({
    transport: "ws",
    workspaceFolders: [startupDirectory]
  }));
  await writeFile(path.join(ideDirectory, "3002.lock"), JSON.stringify({
    transport: "ws",
    workspaceFolders: [sessionDirectory]
  }));
  process.env.CLAUDE_CODE_SSE_PORT = undefined;
  process.env.CLOSEDCODE_EDITOR_SSE_PORT = undefined;
  jest.spyOn(process, "cwd").mockImplementation(() => startupDirectory);
  jest.spyOn(os, "homedir").mockImplementation(() => tmp.path);
...
test("useEditorContext connects with CLOSEDCODE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir();
  process.env.CLAUDE_CODE_SSE_PORT = undefined;
  process.env.CLOSEDCODE_EDITOR_SSE_PORT = "4020";
  jest.spyOn(process, "cwd").mockImplementation(() => tmp.path);
