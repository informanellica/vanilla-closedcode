...
import { useNpmAdd } from "../../fixture/mock-npm.js"
...
test("loads npm tui plugin from package ./tui export", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      ...
      await writeFile(path.join(mod, "server.js"), "export default {}\n");
...
`);
  ...
  const install = useNpmAdd().mockResolvedValue({
...
test("rejects npm tui export that resolves outside plugin directory", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      ...
      await writeFile(path.join(mod, "index.js"), "export default {}\n");
      await writeFile(path.join(outside, "tui.js"), `export default {
  id: "demo.outside",
  tui: async () => {
    await (await import("node:fs/promises")).writeFile(${JSON.stringify(marker)}, "outside")
  },
}
`);
...
test("does not use directory package main for tui entry", async () => {
  await using tmp = await tmpdir({
    init: async dir => {
      ...
      await writeFile(path.join(mod, "main.js"), `export default {
  id: "demo.dir.main",
  tui: async () => {
    await (await import("node:fs/promises")).writeFile(${JSON.stringify(marker)}, "called")
  },
