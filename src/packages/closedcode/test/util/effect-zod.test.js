...
import {  zod, ZodOverride  } from "../../src/util/effect-zod.js"
...
describe("util.effect-zod", () => {
  test("converts class schemas for route dto shapes", () => {
    class Method extends Schema.Class("ProviderAuthMethod")({
  ...
  test("ZodOverride annotation provides the Zod schema for branded IDs", () => {
    ...
    const ID = Schema.String.annotate({
      [ZodOverride]: override
    }).pipe(Schema.brand("TestID"));
  ...
  test("Schema.Class nested in a parent preserves ref via identifier", () => {
    class Inner extends Schema.Class("MyInner")({
      value: Schema.String
    }) {}
    class Outer extends Schema.Class("MyOuter")({
      inner: Inner
    }) {}
    const out = zod(Outer);
    expect(out.meta()?.ref).toBe("MyOuter");
    const shape = out.shape ?? out._def?.shape?.();
    expect(shape.inner.meta()?.ref).toBe("MyInner");
  });
  test("Schema.Class preserves identifier and uses enum format", () => {
    class Rule extends Schema.Class("PermissionRule")({
  ...
  test("ZodOverride on ID carries pattern through Schema.Class", () => {
    ...
    class Request extends Schema.Class("TestRequest")({
      id: ID,
      name: Schema.String
    }) {}
    const schema = json(zod(Request));
    expect(schema.properties.id).toEqual({
      type: "string",
      pattern: "^per.*"
    });
  ...
  test("Permission schemas match original Zod equivalents", () => {
    const MsgID = Schema.String.annotate({
      [ZodOverride]: z.string().startsWith("msg")
    });
    const PerID = Schema.String.annotate({
      [ZodOverride]: z.string().startsWith("per")
    });
    const SesID = Schema.String.annotate({
      [ZodOverride]: z.string().startsWith("ses")
    });
    class Tool extends Schema.Class("PermissionTool")({
      messageID: MsgID,
      callID: Schema.String
    }) {}
    class Request extends Schema.Class("PermissionRequest")({
      id: PerID,
      sessionID: SesID,
      permission: Schema.String,
    ...
    const bridged = json(zod(Request));
    expect(bridged.properties.id).toEqual({
      type: "string",
      pattern: "^per.*"
    });
    expect(bridged.properties.sessionID).toEqual({
      type: "string",
      pattern: "^ses.*"
    });
    expect(bridged.properties.permission).toEqual({
      type: "string"
    });
    ...
    const toolSchema = json(zod(Tool));
    expect(toolSchema.properties.messageID).toEqual({
      type: "string",
      pattern: "^msg.*"
    });
    expect(toolSchema.properties.callID).toEqual({
      type: "string"
    });
  });
  test("ZodOverride survives Schema.brand", () => {
    ...
    class Parent extends Schema.Class("Parent")({
      sessionID: ID
    }) {}
    const schema = json(zod(Parent));
  ...
  describe("Schema.check translation", () => {
    test("filter returning string triggers refinement with that message", () => {
      const isEven = Schema.makeFilter(n => n % 2 === 0 ? undefined : "expected an even number");
    ...
    test("filter returning false triggers refinement with fallback message", () => {
      const nonEmpty = Schema.makeFilter(s => s.length > 0);
    ...
    test("annotations.message on the filter is used when filter returns false", () => {
      const positive = Schema.makeFilter(n => n > 0, {
    ...
    test("cross-field check on a record flags missing key", () => {
      const hasKey = Schema.makeFilter(data => "required" in data ? undefined : "missing 'required' key");
      const schema = zod(Schema.Record(Schema.String, Schema.Struct({
  ...
  describe("StructWithRest / catchall", () => {
    test("struct with a string-keyed record rest parses known AND extra keys", () => {
      const schema = zod(Schema.StructWithRest(Schema.Struct({
    ...
    test("plain struct without rest still emits additionalProperties unchanged (regression)", () => {
      const schema = zod(Schema.Struct({
        id: Schema.String
      }));
      expect(schema.parse({
        id: "x"
  ...
  describe("transforms (Schema.decodeTo)", () => {
    test("Number -> pseudo-Duration (seconds) applies the decode function", () => {
      ...
      const SecondsToMs = Schema.Number.pipe(Schema.decodeTo(Schema.Number, {
        decode: SchemaGetter.transform(n => n * 1000),
        encode: SchemaGetter.transform(ms => ms / 1000)
      }));
    ...
    test("String -> Number via parseInt decode", () => {
      const ParsedInt = Schema.String.pipe(Schema.decodeTo(Schema.Number, {
        decode: SchemaGetter.transform(s => Number.parseInt(s, 10)),
    ...
    test("transform inside a struct field applies per-field", () => {
      const Field = Schema.Number.pipe(Schema.decodeTo(Schema.Number, {
        decode: SchemaGetter.transform(n => n + 1),
        encode: SchemaGetter.transform(n => n - 1)
      }));
      const schema = zod(Schema.Struct({
        plain: Schema.Number,
        bumped: Field
    ...
    test("chained decodeTo composes transforms in order", () => {
      ...
      const Chained = Schema.String.pipe(Schema.decodeTo(Schema.Number, {
        decode: SchemaGetter.transform(s => Number.parseInt(s, 10)),
        encode: SchemaGetter.transform(n => String(n))
      }), Schema.decodeTo(Schema.Number, {
        decode: SchemaGetter.transform(n => n * 2),
        encode: SchemaGetter.transform(n => n / 2)
      }));
    ...
    test("Schema.Class is unaffected by transform walker (returns plain object, not instance)", () => {
      ...
      class Method extends Schema.Class("TxTestMethod")({
        type: Schema.String,
        value: Schema.Number
      }) {}
  ...
  describe("optimizations", () => {
    ...
    test("nested reuse of the same AST reuses the cached Zod child", () => {
      ...
      class Inner extends Schema.Class("MemoTestInner")({
        value: Schema.String
      }) {}
      class OuterA extends Schema.Class("MemoTestOuterA")({
        inner: Inner
      }) {}
      class OuterB extends Schema.Class("MemoTestOuterB")({
        inner: Inner
      }) {}
      const shapeA = zod(OuterA).shape ?? zod(OuterA)._def?.shape?.();
      const shapeB = zod(OuterB).shape ?? zod(OuterB)._def?.shape?.();
    ...
    test("multiple checks run in a single refinement layer (all fire on one value)", () => {
      ...
      const positive = Schema.makeFilter(n => n > 0 ? undefined : "not positive");
      const even = Schema.makeFilter(n => n % 2 === 0 ? undefined : "not even");
      const under100 = Schema.makeFilter(n => n < 100 ? undefined : "too big");
  ...
  describe("well-known refinement translation", () => {
    ...
    test("Schema.isGreaterThan(0) emits exclusiveMinimum: 0", () => {
      ...
      expect(json(schema).exclusiveMinimum).toBe(0);
    ...
    test("Schema.isBetween emits both bounds", () => {
      const schema = zod(Schema.Number.check(Schema.isBetween({
        minimum: 1,
        maximum: 10
      })));
      const shape = json(schema);
    ...
    test("Schema.isMinLength on string emits minLength", () => {
      ...
      expect(json(schema).minLength).toBe(3);
    ...
    test("Schema.isMaxLength on string emits maxLength", () => {
      ...
      expect(json(schema).maxLength).toBe(5);
    ...
    test("Schema.isMinLength on array emits minItems", () => {
      ...
      expect(json(schema).minItems).toBe(1);
    ...
    test("Schema.isPattern emits pattern", () => {
      ...
      expect(json(schema).pattern).toBe("^per");
    ...
    test("Schema.isUUID emits format: uuid", () => {
      ...
      expect(json(schema).format).toBe("uuid");
    });
    test("mix of well-known and anonymous filters translates known and reroutes unknown to superRefine", () => {
      ...
      // back to superRefine.
      const notSeven = Schema.makeFilter(n => n !== 7 ? undefined : "no sevens allowed");
