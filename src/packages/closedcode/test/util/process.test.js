...
describe("util.process", () => {
  ...
  test("returns code when nothrow is enabled", async () => {
    const out = await Process.run(node("process.exit(7)"), {
      nothrow: true
    });
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CLOSEDCODE_TEST ?? "")'), {
      env: {
        CLOSEDCODE_TEST: "set"
      }
    });
  ...
  test("uses shell in run on Windows", async () => {
    if (process.platform !== "win32") return;
    const out = await Process.run(["set", "CLOSEDCODE_TEST_SHELL"], {
      shell: true,
      env: {
    ...
    expect(out.code).toBe(0);
    expect(out.stdout.toString()).toContain("CLOSEDCODE_TEST_SHELL=ok");
  });
