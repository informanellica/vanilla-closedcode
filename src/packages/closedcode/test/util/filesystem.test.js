...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("supports multiple targets with nearest-first default ordering", async () => {
      ...
      expect(result).toEqual([path.join(parent, "cfg.jsonc"), path.join(tmp.path, "cfg.json"), path.join(tmp.path, "cfg.jsonc")]);
    });
    test("supports rootFirst ordering for multiple targets", async () => {
      ...
      expect(result).toEqual([path.join(tmp.path, "cfg.json"), path.join(tmp.path, "cfg.jsonc"), path.join(parent, "cfg.jsonc")]);
    });
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      expect(result).toEqual([path.join(tmp.path, "opencode.json"), path.join(tmp.path, "opencode.jsonc"), path.join(project, "opencode.json"), path.join(project, "opencode.jsonc")]);
    });
  });
  describe("readText()", () => {
