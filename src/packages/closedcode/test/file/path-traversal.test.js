...
import {  containsPath  } from "../../src/project/instance-context.js"
...
const list = dir => run(File.Service.use(svc => svc.list(dir)));
...
describe("File.read path traversal protection", () => {
  test("rejects ../ traversal attempting to read /etc/passwd", async () => {
    await using tmp = await tmpdir({
      init: async dir => {
        await writeFile(path.join(dir, "allowed.txt"), "allowed content");
      }
    });
  ...
  test("rejects deeply nested traversal", async () => {
    ...
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(read("src/nested/../../../../../../../etc/passwd")).rejects.toThrow("Access denied: path escapes project directory");
      }
    });
  });
