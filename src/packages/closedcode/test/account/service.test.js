...
const live = client => Account.layer.pipe(Layer.provide(Layer.succeed(HttpClient.HttpClient, client)));
...
const org = (id, name) => encodeOrg(new Org({
  id: OrgID.make(id),
  name
}));
...
const deviceTokenClient = (body, status = 400) => HttpClient.make(req => Effect.succeed(req.url === "https://one.example.com/auth/device/token" ? json(req, body, status) : json(req, {}, 404)));
const poll = (body, status = 400) => Account.Service.use(s => s.poll(login())).pipe(Effect.provide(live(deviceTokenClient(body, status))));
it.live("login normalizes trailing slashes in the provided server URL", () => Effect.gen(function* () {
  const seen = [];
  const client = HttpClient.make(req => Effect.gen(function* () {
...
it.live("login maps transport failures to account transport errors", () => Effect.gen(function* () {
  const client = HttpClient.make(req => Effect.fail(new HttpClientError.HttpClientError({
    reason: new HttpClientError.TransportError({
      request: req
    })
  })));
  const error = yield* Effect.flip(Account.Service.use(s => s.login("https://one.example.com")).pipe(Effect.provide(live(client))));
...
it.live("orgsByAccount groups orgs per account", () => Effect.gen(function* () {
  yield* AccountRepo.Service.use(r => r.persistAccount({
  ...
  expect(rows.map(row => [row.account.id, row.orgs.map(org => org.id)]).map(([id, orgs]) => [id, orgs])).toEqual([[AccountID.make("user-1"), [OrgID.make("org-1")]], [AccountID.make("user-2"), [OrgID.make("org-2"), OrgID.make("org-3")]]]);
  expect(seen).toEqual(["GET https://one.example.com/api/orgs", "GET https://two.example.com/api/orgs"]);
}));
it.live("token refresh persists the new token", () => Effect.gen(function* () {
  ...
  const client = HttpClient.make(req => Effect.succeed(req.url === "https://one.example.com/auth/device/token" ? json(req, {
    access_token: "at_new",
...
it.live("token refreshes before expiry when inside the eager refresh window", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "user@example.com",
  ...
  let refreshCalls = 0;
  const client = HttpClient.make(req => Effect.promise(async () => {
...
it.live("concurrent config and token requests coalesce token refresh", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "user@example.com",
  ...
  const [cfg, token] = yield* Account.Service.use(s => Effect.all([s.config(id, OrgID.make("org-9")), s.token(id)], {
    concurrency: 2
  })).pipe(Effect.provide(live(client)));
  expect(Option.getOrThrow(cfg)).toEqual({
    theme: "light",
    seats: 5
  });
  expect(String(Option.getOrThrow(token))).toBe("at_new");
...
it.live("config sends the selected org header", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "user@example.com",
...
it.live("poll stores the account and first org on success", () => Effect.gen(function* () {
  const client = HttpClient.make(req => Effect.succeed(req.url === "https://one.example.com/auth/device/token" ? json(req, {
    access_token: "at_1",
    refresh_token: "rt_1",
    token_type: "Bearer",
    expires_in: 60
  }) : req.url === "https://one.example.com/api/user" ? json(req, {
    id: "user-1",
    email: "user@example.com"
  ...
  expect(Option.getOrThrow(active)).toEqual(expect.objectContaining({
    id: "user-1",
    email: "user@example.com",
    active_org_id: "org-1"
  }));
}));
for (const [name, body, expectedTag] of [["pending", {
  ...
  it.live(`poll returns ${name} for ${body.error}`, () => Effect.gen(function* () {
    const result = yield* poll(body);
    expect(result._tag).toBe(expectedTag);
  }));
}
it.live("poll returns poll error for other OAuth errors", () => Effect.gen(function* () {
