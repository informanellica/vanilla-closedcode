...
it.live("active returns none when no accounts exist", () => Effect.gen(function* () {
  const active = yield* AccountRepo.Service.use(r => r.active());
  expect(Option.isNone(active)).toBe(true);
}));
it.live("persistAccount inserts and getRow retrieves", () => Effect.gen(function* () {
...
it.live("persistAccount normalizes trailing slashes in stored server URLs", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
...
it.live("persistAccount sets the active account and org", () => Effect.gen(function* () {
...
it.live("list returns all accounts", () => Effect.gen(function* () {
  const id1 = AccountID.make("user-1");
  const id2 = AccountID.make("user-2");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id: id1,
...
it.live("remove deletes an account", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
...
it.live("use stores the selected org and marks the account active", () => Effect.gen(function* () {
  const id1 = AccountID.make("user-1");
  const id2 = AccountID.make("user-2");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id: id1,
...
it.live("persistToken updates token fields", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
  ...
  yield* AccountRepo.Service.use(r => r.persistToken({
    accountID: id,
...
it.live("persistToken with no expiry sets token_expiry to null", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
...
it.live("persistAccount upserts on conflict", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
...
it.live("remove clears active state when deleting the active account", () => Effect.gen(function* () {
  const id = AccountID.make("user-1");
  yield* AccountRepo.Service.use(r => r.persistAccount({
    id,
    email: "test@example.com",
...
it.live("getRow returns none for nonexistent account", () => Effect.gen(function* () {
