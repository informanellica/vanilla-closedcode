...
const fwd = (...parts) => path.join(...parts).replaceAll("\\", "/");
afterEach(async () => {
...
test("revert should remove new files", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(await fs.access(`${tmp.path}/new.txt`).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
test("revert in subdirectory", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const patch = await run(tmp.path, snapshot => snapshot.patch(before));
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(`${tmp.path}/sub/file.txt`).then(() => true).catch(() => false)).toBe(false);
...
test("multiple file operations", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(await fs.access(`${tmp.path}/c.txt`).then(() => true).catch(() => false)).toBe(false);
      // Note: revert currently only removes files, not directories
...
test("binary file handling", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(`${tmp.path}/image.png`).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
...
test("nested directory revert", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const patch = await run(tmp.path, snapshot => snapshot.patch(before));
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(`${tmp.path}/level1/level2/level3/deep.txt`).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
...
test("revert with empty patches", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(run(tmp.path, snapshot => snapshot.revert([{
        hash: "dummy",
        files: []
...
test("unicode filenames", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(before).toBeTruthy();
      const unicodeFiles = [{
        path: fwd(tmp.path, "文件.txt"),
        content: "chinese content"
      }, {
        path: fwd(tmp.path, "🚀rocket.txt"),
        content: "emoji content"
      }, {
        path: fwd(tmp.path, "café.txt"),
        content: "accented content"
      }, {
        path: fwd(tmp.path, "файл.txt"),
        content: "cyrillic content"
      }];
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      for (const file of unicodeFiles) {
        expect(await fs.access(file.path).then(() => true).catch(() => false)).toBe(false);
      }
    }
  });
});
...
test("unicode filenames in subdirectories", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(deepFile).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
test("very long filenames", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(longFile).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
...
test("git info exclude changes", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await writeFile(`${tmp.path}/ignored.txt`, "ignored content");
      await writeFile(`${tmp.path}/normal.txt`, "normal content");
      const patch = await run(tmp.path, snapshot => snapshot.patch(before));
      expect(patch.files).toContain(fwd(tmp.path, "normal.txt"));
...
test("git info exclude keeps global excludes", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await writeFile(global, "global.tmp\n");
      ...
      try {
        ...
        const text = await readText(file);
        await writeFile(file, `${text.trimEnd()}\ninfo.tmp\n`);
        await writeFile(`${tmp.path}/global.tmp`, "global content");
        await writeFile(`${tmp.path}/info.tmp`, "info content");
        await writeFile(`${tmp.path}/normal.txt`, "normal content");
        const patch = await run(tmp.path, snapshot => snapshot.patch(before));
        expect(patch.files).toContain(fwd(tmp.path, "normal.txt"));
        ...
        if (prev) process.env.GIT_CONFIG_GLOBAL = prev;else delete process.env.GIT_CONFIG_GLOBAL;
      }
    }
  });
});
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        expect(await fs.access(worktreeFile).then(() => true).catch(() => false)).toBe(false);
      }
    });
...
test("restore function", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(await fs.access(`${tmp.path}/a.txt`).then(() => true).catch(() => false)).toBe(true);
      expect(await fs.readFile(`${tmp.path}/a.txt`, "utf-8")).toBe(tmp.extra.aContent);
      expect(await fs.access(`${tmp.path}/new.txt`).then(() => true).catch(() => false)).toBe(true); // New files should remain
      expect(await fs.readFile(`${tmp.path}/b.txt`, "utf-8")).toBe(tmp.extra.bContent);
    }
  });
});
test("revert should not delete files that existed but were deleted in snapshot", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(`${tmp.path}/a.txt`).then(() => true).catch(() => false)).toBe(false);
    }
  });
});
test("revert preserves file that existed in snapshot when deleted then recreated", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      expect(await fs.access(`${tmp.path}/newfile.txt`).then(() => true).catch(() => false)).toBe(false);
      expect(await fs.access(`${tmp.path}/existing.txt`).then(() => true).catch(() => false)).toBe(true);
...
test("diffFull with a large interleaved mixed diff", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await Promise.all([...mod.map((file, i) => Filesystem.write(file, `before-${ids[i]}-é\n🙂\nline`)), ...del.map((file, i) => Filesystem.write(file, `gone-${ids[i]}\n你好`)), ...bin.map((file, i) => Filesystem.write(file, new Uint8Array([0, i, 255, i % 251])))]);
      const before = await run(tmp.path, snapshot => snapshot.track());
      expect(before).toBeTruthy();
      await Promise.all([...mod.map((file, i) => Filesystem.write(file, `after-${ids[i]}-é\n🚀\nline`)), ...add.map((file, i) => Filesystem.write(file, `new-${ids[i]}\nこんにちは`)), ...bin.map((file, i) => Filesystem.write(file, new Uint8Array([9, i, 8, i % 251]))), ...del.map(file => fs.rm(file))]);
      const after = await run(tmp.path, snapshot => snapshot.track());
      expect(after).toBeTruthy();
      const diffs = await run(tmp.path, snapshot => snapshot.diffFull(before, after));
...
test("revert preserves patch order when the same hash appears again", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      await run(tmp.path, snapshot => snapshot.revert([{
        hash: snap1,
        files: [fwd(tmp.path, "a.txt")]
      }, {
        hash: snap2,
        files: [fwd(tmp.path, "foo")]
      }, {
        hash: snap1,
        files: [fwd(tmp.path, "foo", "bar")]
      }]));
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      const base = Array.from({
        length: 140
      }, (_, i) => fwd(tmp.path, "batch", `${i}.txt`));
      const fresh = Array.from({
        length: 140
      }, (_, i) => fwd(tmp.path, "fresh", `${i}.txt`));
      ...
      await run(tmp.path, snapshot => snapshot.revert([patch]));
      await Promise.all(base.map(async (file, i) => {
        expect(await fs.readFile(file, "utf-8")).toBe(`base-${i}`);
      }));
      await Promise.all(fresh.map(async file => {
