...
import { fileExists, writeFile } from "../lib/io.js";

void Log.init({
  print: false
});
const encoder = new TextEncoder();
function run(fn, layer = Project.defaultLayer) {
 ...
 */
function mockGitFailure(failArg) {
  return Layer.effect(ChildProcessSpawner.ChildProcessSpawner, Effect.gen(function* () {
    const real = yield* ChildProcessSpawner.ChildProcessSpawner;
    return ChildProcessSpawner.make(Effect.fnUntraced(function* (command) {
...
function projectLayerWithFailure(failArg) {
  return Project.layer.pipe(Layer.provide(mockGitFailure(failArg)), Layer.provide(Bus.defaultLayer), Layer.provide(AppFileSystem.defaultLayer), Layer.provide(NodePath.layer));
}
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const cacheFile = path.join(tmp.path, ".git", "closedcode");
    expect(await fileExists(cacheFile)).toBe(false);
  });
...
describe("Project.fromDirectory git failure paths", () => {
  ...
  test("handles show-toplevel failure gracefully", async () => {
    ...
    const {
      project,
      sandbox
    } = await run(svc => svc.fromDirectory(tmp.path), layer);
    expect(project.worktree).toBe(tmp.path);
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("should set worktree to root when called from a worktree", async () => {
    ...
    try {
      ...
      const {
        project,
        sandbox
      } = await run(svc => svc.fromDirectory(worktreePath));
      expect(project.worktree).toBe(tmp.path);
      expect(sandbox).toBe(worktreePath);
      expect(project.sandboxes).toContain(worktreePath);
      expect(project.sandboxes).not.toContain(tmp.path);
    } finally {
      await $`git worktree remove ${worktreePath}`.cwd(tmp.path).quiet().catch(() => {});
    }
  });
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "closedcode");
      const exists = await fileExists(cache);
  ...
  test("separate clones of the same repo should share project ID", async () => {
    ...
    try {
      ...
      const {
        project: b
      } = await run(svc => svc.fromDirectory(clone));
      expect(b.id).toBe(a.id);
    } finally {
  ...
  test("should accumulate multiple worktrees in sandboxes", async () => {
    ...
    try {
      ...
      const {
        project
      } = await run(svc => svc.fromDirectory(worktree2));
      expect(project.worktree).toBe(tmp.path);
      expect(project.sandboxes).toContain(worktree1);
      expect(project.sandboxes).toContain(worktree2);
      expect(project.sandboxes).not.toContain(tmp.path);
    } finally {
      await $`git worktree remove ${worktree1}`.cwd(tmp.path).quiet().catch(() => {});
      await $`git worktree remove ${worktree2}`.cwd(tmp.path).quiet().catch(() => {});
    }
  });
});
...
describe("Project.update", () => {
  test("should update name", async () => {
    ...
    const updated = await run(svc => svc.update({
      projectID: project.id,
  ...
  test("should throw error when project not found", async () => {
    await expect(run(svc => svc.update({
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const correctCache = path.join(barePath, "closedcode");
      const wrongCache = path.join(parentDir, ".git", "closedcode");
      expect(await fileExists(correctCache)).toBe(true);
      expect(await fileExists(wrongCache)).toBe(false);
    } finally {
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const {
        project: projA
      } = await run(svc => svc.fromDirectory(worktreeA));
      const {
        project: projB
      } = await run(svc => svc.fromDirectory(worktreeB));
      expect(projA.id).not.toBe(projB.id);
      const cacheA = path.join(bareA, "closedcode");
      const cacheB = path.join(bareB, "closedcode");
      const wrongCache = path.join(parentDir, ".git", "closedcode");
      expect(await fileExists(cacheA)).toBe(true);
      expect(await fileExists(cacheB)).toBe(true);
      expect(await fileExists(wrongCache)).toBe(false);
    } finally {
