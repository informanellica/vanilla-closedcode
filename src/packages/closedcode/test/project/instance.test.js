...
import {  registerDisposer  } from "../../src/effect/instance-registry.js"
...
let bootstrapRun = Effect.void;
const noopBootstrap = Layer.succeed(InstanceBootstrap.Service, InstanceBootstrap.Service.of({
  run: Effect.suspend(() => bootstrapRun)
}));
const it = testEffect(Layer.mergeAll(InstanceStore.defaultLayer, CrossSpawnSpawner.defaultLayer).pipe(Layer.provide(noopBootstrap)));
afterEach(async () => {
  bootstrapRun = Effect.void;
  await disposeAllInstances();
});
describe("InstanceStore", () => {
  it.live("loads instance context without installing ALS for the caller", () => Effect.gen(function* () {
  ...
  it.live("runs bootstrap with InstanceRef provided", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
  ...
  it.live("caches loaded instance context by directory", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
  ...
  it.live("dedupes concurrent loads while init is in flight", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
  ...
  it.live("removes failed loads from the cache", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
  ...
  it.live("reload replaces the cached context", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
    ...
    const second = yield* store.reload({
      directory: dir
    });
  ...
  it.live("stale dispose does not delete an in-flight reload", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
    ...
    const reload = yield* store.reload({
      directory: dir
    }).pipe(Effect.forkScoped);
    ...
    expect(yield* store.load({
      directory: dir
    })).toBe(second);
  }));
  it.live("dedupes concurrent disposeAll calls", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const store = yield* InstanceStore.Service;
    const disposing = Promise.withResolvers();
    const releaseDispose = Promise.withResolvers();
    const disposed = [];
    const off = registerDisposer(async directory => {
      disposed.push(directory);
      disposing.resolve();
  ...
  it.live("re-arms disposeAll after completion", () => Effect.gen(function* () {
  ...
  it.live("provides legacy Promise callers with instance ALS", () => Effect.gen(function* () {
    const dir = yield* tmpdirScoped({
      git: true
    });
    const directory = yield* Effect.promise(() => WithInstance.provide({
      directory: dir,
