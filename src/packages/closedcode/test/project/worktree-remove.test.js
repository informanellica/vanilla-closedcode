...
describe("Worktree.remove", () => {
  it.live("continues when git remove exits non-zero after detaching", () => provideTmpdirInstance(root => Effect.gen(function* () {
    ...
    yield* Effect.promise(() => writeFile(shim, ["#!/bin/bash", `REAL_GIT=${JSON.stringify(real)}`, 'if [ "$1" = "worktree" ] && [ "$2" = "remove" ]; then', '  "$REAL_GIT" "$@" >/dev/null 2>&1', '  echo "fatal: failed to remove worktree: Directory not empty" >&2', "  exit 1", "fi", 'exec "$REAL_GIT" "$@"'].join("\n")));
    yield* Effect.promise(() => fs.chmod(shim, 0o755));
    const prev = yield* Effect.acquireRelease(Effect.sync(() => {
    ...
    const ref = yield* Effect.promise(() => $`git show-ref --verify --quiet refs/heads/${branch}`.cwd(root).quiet().nothrow());
    expect(ref.exitCode).not.toBe(0);
  }), {
    git: true
  }));
  wintest("stops fsmonitor before removing a worktree", () => provideTmpdirInstance(root => Effect.gen(function* () {
    const svc = yield* Worktree.Service;
    ...
    yield* Effect.promise(() => writeFile(path.join(dir, "tracked.txt"), "next\n"));
