...
async function waitReady() {
  const {
    GlobalBus
  } = await import("../../src/bus/global.js");
...
describe("Worktree", () => {
  ...
  describe("makeWorktreeInfo", () => {
    it.live("returns info with name, branch, and directory", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
      const info = yield* svc.makeWorktreeInfo();
      expect(info.name).toBeDefined();
      expect(typeof info.name).toBe("string");
      expect(info.branch).toBe(`closedcode/${info.name}`);
      expect(info.directory).toContain(info.name);
    }), {
      git: true
    }));
    it.live("uses provided name as base", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
      const info = yield* svc.makeWorktreeInfo("my-feature");
      expect(info.name).toBe("my-feature");
      expect(info.branch).toBe("closedcode/my-feature");
    }), {
      git: true
    }));
    it.live("slugifies the provided name", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
    ...
    it.live("throws NotGitError for non-git directories", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
  ...
  describe("create + remove lifecycle", () => {
    it.live("create returns worktree info and remove cleans up", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
    ...
    it.live("create returns after setup and fires Event.Ready after bootstrap", () => provideTmpdirInstance(dir => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
    ...
    it.live("create with custom name", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
      const ready = waitReady();
      const info = yield* svc.create({
        name: "test-workspace"
      ...
      expect(info.branch).toBe("closedcode/test-workspace");
  ...
  describe("createFromInfo", () => {
    wintest("creates and bootstraps git worktree", () => provideTmpdirInstance(dir => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
  ...
  describe("remove edge cases", () => {
    it.live("remove non-existent directory succeeds silently", () => provideTmpdirInstance(dir => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
      const ok = yield* svc.remove({
    ...
    it.live("throws NotGitError for non-git directories", () => provideTmpdirInstance(() => Effect.gen(function* () {
      const svc = yield* Worktree.Service;
      const exit = yield* Effect.exit(svc.remove({
        directory: "/tmp/fake"
      }));
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) expect(Cause.squash(exit.cause)).toBeInstanceOf(Worktree.NotGitError);
