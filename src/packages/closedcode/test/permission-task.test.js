...
describe("Permission.evaluate for permission.task", () => {
  const createRuleset = rules => Object.entries(rules).map(([pattern, action]) => ({
    permission: "task",
    pattern,
    action
  }));
  ...
  test("matches wildcard patterns with ask", () => {
    ...
    const globalRuleset = createRuleset({
      "*": "ask"
    });
  ...
  test("matches global wildcard", () => {
    expect(Permission.evaluate("task", "any-agent", createRuleset({
      "*": "allow"
    })).action).toBe("allow");
    expect(Permission.evaluate("task", "any-agent", createRuleset({
      "*": "deny"
...
describe("permission.task with real config files", () => {
  test("loads task permissions from closedcode.json config", async () => {
