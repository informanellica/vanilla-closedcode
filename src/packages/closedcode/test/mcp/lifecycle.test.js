...
function getOrCreateClientState(name) {
...
function withInstance(config, fn) {
  return async () => {
...
test("tools() reuses cached tool definitions after connect", withInstance({}, mcp => Effect.gen(function* () {
  ...
  expect(addResult.status["my-server"]?.status ?? addResult.status.status).toBe("connected");
...
test("tool change notifications refresh cached tool definitions", withInstance({}, mcp => Effect.gen(function* () {
...
test("disconnect sets status to disabled and removes client", withInstance({
  "disc-server": {
    type: "local",
    command: ["echo", "test"]
  }
}, mcp => Effect.gen(function* () {
...
test("connect() after disconnect() re-establishes the server", withInstance({
  "reconn-server": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("init connects available servers even when one fails", withInstance({
  "good-server": {
    type: "local",
    command: ["echo", "good"]
  },
...
test("disabled server is marked as disabled without attempting connection", withInstance({
  "disabled-server": {
    type: "local",
    command: ["echo", "test"],
    enabled: false
...
test("prompts() returns prompts from connected servers", withInstance({
  "prompt-server": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("resources() returns resources from connected servers", withInstance({
  "resource-server": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("prompts() skips disconnected servers", withInstance({
  "prompt-disc-server": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("connect() on nonexistent server does not throw", withInstance({}, mcp => Effect.gen(function* () {
  // Should not throw
...
test("disconnect() on nonexistent server does not throw", withInstance({}, mcp => Effect.gen(function* () {
...
test("tools() returns empty when no MCP servers are configured", withInstance({}, mcp => Effect.gen(function* () {
  const tools = yield* mcp.tools();
...
test("server that fails to connect is marked as failed", withInstance({
  "fail-connect": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("tools() prefixes tool names with sanitized server name", withInstance({
  "my.special-server": {
    type: "local",
    command: ["echo", "test"]
  }
...
test("local stdio transport is closed when connect times out (no process leak)", withInstance({}, mcp => Effect.gen(function* () {
  ...
  const serverStatus = addResult.status["hanging-server"] ?? addResult.status;
...
test("remote transport is closed when connect times out", withInstance({}, mcp => Effect.gen(function* () {
  ...
  const serverStatus = addResult.status["hanging-remote"] ?? addResult.status;
  expect(serverStatus.status).toBe("failed");
...
test("failed remote transport is closed before trying next transport", withInstance({}, mcp => Effect.gen(function* () {
  ...
  const serverStatus = addResult.status["fail-remote"] ?? addResult.status;
  expect(serverStatus.status).toBe("failed");
