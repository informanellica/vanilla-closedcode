...
test("first connect to OAuth server shows needs_auth instead of failed", async () => {
  ...
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      const result = await Effect.runPromise(MCP.Service.use(mcp => mcp.add("test-oauth", {
        type: "remote",
        url: "https://example.com/mcp"
...
test("state() generates a new state when none is saved", async () => {
  const {
    McpOAuthProvider
  } = await import("../../src/mcp/oauth-provider.js");
  const {
    McpAuth
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const provider = new McpOAuthProvider("test-state-gen", "https://example.com/mcp", {}, {
        onRedirect: async () => {}
      }, auth);
      const entryBefore = await Effect.runPromise(McpAuth.Service.use(auth => auth.get("test-state-gen")).pipe(Effect.provide(McpAuth.defaultLayer)));
      ...
      const entryAfter = await Effect.runPromise(McpAuth.Service.use(auth => auth.get("test-state-gen")).pipe(Effect.provide(McpAuth.defaultLayer)));
...
test("state() returns existing state when one is saved", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const provider = new McpOAuthProvider("test-state-existing", "https://example.com/mcp", {}, {
        onRedirect: async () => {}
      }, auth);

      // Pre-save a state
      const existingState = "pre-saved-state-value";
      await Effect.runPromise(McpAuth.Service.use(auth => auth.updateOAuthState("test-state-existing", existingState)).pipe(Effect.provide(McpAuth.defaultLayer)));
