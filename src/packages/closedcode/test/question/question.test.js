...
import {  QuestionID  } from "../../src/question/schema.js"
...
const rejectEffect = Effect.fn("QuestionTest.reject")(function* (id) {
  const question = yield* Question.Service;
...
const rejectAll = Effect.gen(function* () {
  yield* Effect.forEach(yield* listEffect, req => rejectEffect(req.id), {
    discard: true
  });
});
...
it.instance("ask - remains pending until answered", () => Effect.gen(function* () {
...
it.instance("ask - adds to pending list", () => Effect.gen(function* () {
  const questions = [{
    question: "What would you like to do?",
    header: "Action",
    options: [{
...
it.instance("reply - resolves the pending ask with answers", () => Effect.gen(function* () {
  const questions = [{
    question: "What would you like to do?",
    header: "Action",
    options: [{
...
it.instance("reply - removes from pending list", () => Effect.gen(function* () {
  const fiber = yield* askEffect({
    sessionID: SessionID.make("ses_test"),
    questions: [{
      question: "What would you like to do?",
...
it.instance("reply - does nothing for unknown requestID", () => replyEffect({
...
it.instance("reject - throws RejectedError", () => Effect.gen(function* () {
  const fiber = yield* askEffect({
    sessionID: SessionID.make("ses_test"),
    questions: [{
      question: "What would you like to do?",
...
it.instance("reject - removes from pending list", () => Effect.gen(function* () {
  const fiber = yield* askEffect({
    sessionID: SessionID.make("ses_test"),
    questions: [{
      question: "What would you like to do?",
...
it.instance("ask - handles multiple questions", () => Effect.gen(function* () {
  const questions = [{
    question: "What would you like to do?",
    header: "Action",
    options: [{
...
it.instance("list - returns all pending requests", () => Effect.gen(function* () {
...
it.instance("list - returns empty when no pending", () => Effect.gen(function* () {
  const pending = yield* listEffect;
  expect(pending.length).toBe(0);
}), {
  git: true
});
it.live("questions stay isolated by directory", () => Effect.gen(function* () {
...
it.live("pending question rejects on instance dispose", () => Effect.gen(function* () {
...
it.live("pending question rejects on instance reload", () => Effect.gen(function* () {
  const dir = yield* tmpdirScoped({
    git: true
  });
  const fiber = yield* askEffect({
